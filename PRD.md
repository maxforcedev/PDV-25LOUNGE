...
- [ ] Corrigir ProductComponent `UN`: usar `min=1`, `step=1` e aceitar `5` sem exigir `5,001`.
- [ ] Manter `min/step` decimal adequado para KG/G/L/ML.
- [ ] Padronizar inputs para aceitar `,` e `.` em todo o sistema.
- [ ] Corrigir checkout em dinheiro para o operador informar somente **Valor recebido**.
- [ ] Calcular `Payment.amount` pelo saldo restante e troco pelo backend.
- [ ] Validar pagamento dividido com dinheiro sem campo redundante.

### Caixa e cancelamentos

- [ ] Corrigir Histórico da Sessão de Caixa.
- [ ] Compor timeline com abertura, entradas, sangrias, vendas em dinheiro, consumações cobradas, cancelamentos e fechamento.
- [ ] Tornar vendas/consumações da timeline clicáveis.
- [ ] Bloquear cancelamento quando a CashSession original estiver fechada.
- [ ] Validar o bloqueio também diretamente pela API.

### Estoque

- [ ] Expor número/tipo da Sale relacionada ao StockMovement.
- [ ] Substituir visualmente “Motivo” por **Origem / Detalhes** nos movimentos automáticos.
- [ ] Adicionar popover/dropdown com detalhes e link para abrir venda/consumação.

### Relatórios

- [ ] Remover chaves técnicas/labels em inglês da UI.
- [ ] Criar labels explícitos pt-BR.
- [ ] Remover duplicidade `discount`/`manual_discount`.
- [ ] Exibir Desconto manual, Desconto promocional e Descontos totais.
- [ ] Adicionar Valor bruto a preço de tabela e Faturamento efetivo.
- [ ] Corrigir ranking de Product/Category para ratear desconto manual e reconciliar totais.
- [ ] Corrigir relatório de CashSession para considerar interseção com o período.
- [ ] Simplificar `/consumacoes` com Busca + Filtros compactos.
- [ ] Fazer `/estoque` ser a visão oficial do estoque atual e manter período apenas no histórico de movimentações.

### Dashboard

- [ ] Tornar KPIs clicáveis.
- [ ] Preservar período/filtros na navegação para Vendas, Estoque, Sangrias e demais telas relacionadas.
- [ ] Revisar layout dos KPIs conforme design system.
...
- [ ] Build do frontend.
- [ ] Validar manualmente os cenários corrigidos.
...
- [ ] Adicionar escopo por Branch e opção Todas as filiais.
- [ ] Tornar `ends_at` opcional.
- [ ] Criar agenda semanal por dia da semana.
- [ ] Permitir múltiplos intervalos por dia.
- [ ] Suportar intervalo atravessando meia-noite com regra explícita.
- [ ] Se não houver agenda semanal, considerar promoção válida durante todo o dia dentro da vigência.
- [ ] Definir desconto fixo como benefício **por unidade**.
- [ ] Detectar conflito Product × Product.
- [ ] Detectar conflito Product × Category.
- [ ] Detectar conflito Category × Category quando houver sobreposição efetiva de Products.
- [ ] Detectar conflito Branch específica × Todas as filiais.
- [ ] Detectar conflito considerando data, weekday e intervalo horário.
- [ ] Bloquear conflito no backend ao criar, editar ou ativar promoção.
- [ ] Exibir mensagem de conflito clara indicando promoção, alvo, Branch e horário envolvidos.
- [ ] Atualizar checkout para refletir agenda/Branch corretamente.
- [ ] Preservar histórico da promoção aplicada.
...
- [ ] Criar `BranchSettings` one-to-one com Branch.
- [ ] Adicionar `allow_negative_stock` padrão `False`.
- [ ] Adicionar configurações previstas de taxa de serviço, comissão e custo fixo, mesmo que algumas só sejam consumidas nas sprints seguintes.
- [ ] Criar API e tela de configurações da Branch.
- [ ] Na tela de Empresa, listar Branches e resumo das configurações.

### Estoque negativo

- [ ] Com `allow_negative_stock=False`, manter bloqueio por saldo insuficiente.
- [ ] Com `allow_negative_stock=True`, permitir saldo negativo real.
- [ ] Adicionar estado `negative`.
- [ ] Atualizar filtros/widgets para negativo.
- [ ] Aplicar regra também aos componentes de combos.

### Preço por filial

- [ ] Criar `BranchProductPrice`.
- [ ] Manter `Product.sale_price` como preço padrão da Company.
- [ ] Resolver preço efetivo no backend por Branch com fallback para Product.
- [ ] Atualizar cálculo do PDV e Promoções para usar preço efetivo da Branch.
- [ ] Preservar snapshot histórico no SaleItem.
- [ ] Criar tela/relatório comparativo de preços `Produto | Filial X | Filial Y`.
...
- [ ] Adicionar `seller_user` obrigatório em toda venda comercial.
- [ ] Não permitir opção “Sem atendente”.
- [ ] Listar somente Users elegíveis da Branch com permissão de venda.
- [ ] Permitir garçom, bartender, gerente, caixa ou outro User autorizado.
- [ ] Pré-selecionar o próprio operador quando elegível, sem ocultar a informação.
- [ ] Manter `created_by` separado de `seller_user`.

### Autorização de desconto

- [ ] Manter ação de desconto visível para operador sem permissão, sinalizando necessidade de autorização.
- [ ] Criar endpoint para listar autorizadores elegíveis da Branch.
- [ ] Exibir select com Users que possuem `sales.apply_discount`.
- [ ] Solicitar senha do autorizador na primeira versão; preparar evolução futura para PIN.
- [ ] Validar credencial/permissão no backend sem trocar sessão.
- [ ] Registrar `discount_approved_by` na Sale.

### Taxa de serviço

- [ ] Configurar taxa de serviço por Branch.
- [ ] Calcular sobre subtotal líquido após promoções e desconto manual.
- [ ] Exibir claramente no checkout.
- [ ] Salvar taxa e valor como snapshot da Sale.

### Comissão

- [ ] Configurar percentual padrão de comissão por Branch.
- [ ] Comissão pode ser menor, igual ou maior que a taxa de serviço, sem vínculo automático entre percentuais.
- [ ] Calcular comissão sobre subtotal líquido após promoções e desconto manual na V1.
- [ ] Salvar percentual e valor como snapshot da Sale.
- [ ] Associar comissão ao `seller_user`.
- [ ] Cancelamento válido deve remover/reverter comissão da operação.
...
- [ ] Adicionar gráfico de faturamento/vendas por hora.
- [ ] Adicionar gráfico de formas de pagamento.
- [ ] Adicionar ranking/gráfico de Products.
- [ ] Adicionar ranking/gráfico por atendente.
- [ ] Manter KPIs clicáveis e design system.

### Vendas e comissão

- [ ] Filtro por operador.
- [ ] Filtro por atendente.
- [ ] Agrupar vendas/faturamento por operador.
- [ ] Agrupar vendas/faturamento por atendente.
- [ ] KPI de comissão.
- [ ] KPI de taxa de serviço cobrada.
- [ ] Permitir detalhar as vendas que compõem cada agrupamento.

### Valor bruto e descontos

- [ ] KPI Valor bruto a preço de tabela.
- [ ] KPI Desconto manual.
- [ ] KPI Desconto promocional.
- [ ] KPI Descontos totais.
- [ ] KPI Faturamento efetivo.
- [ ] Garantir `valor_bruto = faturamento_efetivo + descontos_totais` no escopo considerado, respeitando taxa de serviço separadamente.

### Resultado operacional estimado

- [ ] Calcular CMV com `SaleItem.unit_cost` histórico.
- [ ] Subtrair comissão histórica.
- [ ] Criar classificação explícita de sangria/movimento que afeta ou não resultado.
- [ ] Não tratar transferência para cofre como despesa.
- [ ] Adicionar custo fixo diário/configurável da Branch.
- [ ] Calcular custo fixo rateado pelo período/regra definida.
- [ ] Exibir Resultado estimado e Margem estimada.
- [ ] Permitir relatório por período e, quando útil, por CashSession como proxy do evento/noite.
- [ ] Não chamar o relatório de DRE contábil.
...
- [ ] Implementar modo claro/escuro seguindo design system.
- [ ] Persistir preferência do tema.
- [ ] Implementar tela cheia quando suportado.
- [ ] Implementar sidebar recolhível/expansível no desktop.
- [ ] Manter drawer responsivo no mobile.
- [ ] Revisar relatórios e dashboards em desktop/tablet/mobile.
- [ ] Revisar densidade visual, tabelas, gráficos, filtros e estados vazios.
- [ ] Garantir UI final 100% pt-BR.
- [ ] Remover restos de labels técnicas/inglês exibidos ao usuário.
...
- [ ] Permissões, incluindo Administrador e perfis restritos.
