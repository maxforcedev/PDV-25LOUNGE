...
> **Versão:** 1.1  
> **Data:** 2026-08-15  
> **Status:** PRD técnico — MVP atualizado com RBAC configurável, comportamento de estoque, composição básica e pagamentos configuráveis  
...
O MVP deve cobrir o núcleo operacional necessário para que uma empresa consiga configurar sua estrutura básica, criar perfis de acesso e permissões por empresa, cadastrar produtos simples, não estocáveis ou compostos, controlar estoque por filial com estoque mínimo, configurar formas de pagamento, abrir caixa, realizar uma venda, registrar o pagamento, baixar o estoque correto conforme o comportamento do produto, consultar ou cancelar a venda, movimentar o caixa, fechar o caixa e acompanhar indicadores básicos no dashboard.
...
- Implementar autorização por perfil configurável por Company, sem regras hardcoded pelo nome do perfil.
...
- Perfis de acesso pertencem à empresa e determinam permissões funcionais sem depender do nome fixo do perfil.
...
Cada perfil deve possuir um conjunto explícito de permissões funcionais. Um usuário pode possuir perfil diferente em empresas diferentes por meio de seu acesso à Company.
...
21. Perfis de acesso são configuráveis por Company; nomes de perfil não são fonte de autorização.
22. Produtos `direct` baixam seu próprio estoque.
23. Produtos `none` não consultam, baixam ou devolvem estoque por venda.
24. Produtos `components` baixam exclusivamente os componentes configurados, sem baixar o próprio produto pai.
25. Composição do MVP é de um nível: componentes não podem depender recursivamente de outra composição.
26. O cancelamento deve reverter os `StockMovement` realmente criados pela venda original.
27. Estoque mínimo pertence ao contexto `product + branch`, não ao Product global.
28. Somente formas de pagamento ativas da Company podem ser selecionadas em novas vendas.
...
```text
User
├── Company A — Perfil: Administrador
│   ├── Branch A1
│   └── Branch A2
└── Company B — Perfil: Operador de Caixa
    └── Branch B1
```
...
- permita associar um `AccessProfile` específico ao acesso do User em cada Company;
- permita que o mesmo User possua perfis diferentes em Companies diferentes;
- permita filtrar autorização no backend por perfil, permissão e Branch;
...
erDiagram
    ...
    ACCESS_PROFILE ||--o{ USER_COMPANY_ACCESS : assigned_to
...
- `UserCompanyAccess` deve apontar para o perfil efetivo da relação usuário ↔ Company ou modelagem equivalente que preserve essa capacidade.
...
- produtos com `inventory_behavior='none'` não precisam participar da disponibilidade de venda e não devem gerar baixa por venda.
...
A composição operacional de estoque não precisa ser duplicada integralmente em `SaleItem` se os `StockMovement` vinculados à venda preservarem com clareza os itens realmente consumidos. O histórico nunca deve depender da composição atual do Product.
...
- associar o perfil ao acesso do usuário naquela Company;
- permitir que o mesmo User tenha perfis distintos em Companies distintas;
...
2. resolver o contexto autorizado do usuário;
3. obter o perfil efetivo do usuário naquela Company;
4. validar a permissão funcional necessária;
5. validar acesso à Company;
6. validar acesso à Branch quando aplicável;
7. validar pertencimento do objeto;
8. impedir acesso cruzado por manipulação de ID;
9. filtrar listagens no backend.
...
## 19.4 Produto vendável e insumo
...
Exemplos possíveis: taxa, serviço, ingresso sem controle de inventário físico.
...
- dashboard deve diferenciar, quando aplicável, estoque zerado e estoque abaixo do mínimo.

## 20.4 Movimentações manuais
...
## 20.8 Não implementar

- transferência;
- lote;
- validade;
- inventário avançado;
- consumo interno dedicado;
- fornecedor;
- compra;
- previsão de estoque;
- reserva de estoque;
...
- usuário responsável deve estar autenticado e autorizado pela permissão efetiva do perfil naquela Company;
- item preserva preço histórico;
- backend recalcula todos os valores;
- somente produtos `is_sellable=True` podem ser adicionados normalmente ao PDV;
...
Caso o MVP permita múltiplos pagamentos por venda, a soma deve ser validada pelo backend. Caso a primeira versão operacional limite a um método por venda, essa simplificação deve ser documentada sem quebrar a modelagem `Sale -> Payment[]`.

## 23.3 Configuração no frontend
...
2. permissão efetiva de realizar venda;
...
13. comportamento de estoque carregado do backend;
14. composição válida para produtos `components`;
15. estoques físicos necessários resolvidos pelo backend;
16. estoque suficiente dos produtos/componentes efetivamente consumidos;
17. PaymentMethods existentes, ativos e pertencentes à Company;
18. pagamentos válidos;
19. total recalculado;
20. ausência de inconsistência concorrente.
...
6. recalcular preços, subtotais, desconto e total;
7. validar PaymentMethods e pagamentos;
8. criar ou atualizar registro da venda conforme desenho escolhido;
9. criar itens com preço histórico;
10. registrar pagamentos;
11. reduzir os estoques físicos resolvidos;
12. criar `StockMovement(type='sale')` para cada baixa real, vinculando-os à Sale;
13. marcar venda como finalizada;
14. commit.
...
Regras de estoque no dashboard:

- considerar o contexto de Branch;
- `minimum_quantity` vem de `Stock`;
- produtos `inventory_behavior='none'` não devem gerar alerta de disponibilidade física de venda;
- produtos não vendáveis podem aparecer em alerta de estoque quando forem insumos utilizados por composições.
...
- menus e ações devem ser exibidos de acordo com as permissões efetivas do perfil no contexto da Company atual;
...
- `Sobre mim` exibe dados pessoais, empresas, perfil e filiais vinculadas;
...
| Perfil baseado em nome fixo | autorização incorreta e baixa flexibilidade | AccessProfile por Company + permissões efetivas |
...
- [ ] AC-05 — Criar usuário com perfil em uma Company e acesso a Branch autorizada.
- [ ] AC-06 — Realizar login e aplicar permissões sem depender do nome do perfil.
- [ ] AC-07 — Cadastrar categoria.
- [ ] AC-08 — Cadastrar produto `direct` e configurar estoque mínimo por Branch.
- [ ] AC-09 — Cadastrar produto `none` e vendê-lo sem movimentar estoque.
- [ ] AC-10 — Cadastrar insumo não vendável.
- [ ] AC-11 — Cadastrar produto `components` com composição básica válida.
- [ ] AC-12 — Adicionar estoque aos produtos físicos/insumos.
- [ ] AC-13 — Configurar formas de pagamento padrão, ativando/inativando métodos.
- [ ] AC-14 — Abrir caixa.
- [ ] AC-15 — Acessar PDV e visualizar somente produtos vendáveis.
- [ ] AC-16 — Adicionar produtos à venda.
- [ ] AC-17 — Selecionar apenas forma de pagamento ativa.
- [ ] AC-18 — Finalizar venda.
- [ ] AC-19 — Produto `direct` baixar seu próprio estoque.
- [ ] AC-20 — Produto `none` não movimentar estoque.
- [ ] AC-21 — Produto `components` baixar corretamente seus componentes, sem baixar o produto pai.
- [ ] AC-22 — Visualizar venda realizada e movimentos de estoque associados.
- [ ] AC-23 — Cancelar venda e devolver exatamente os produtos/quantidades movimentados originalmente, mesmo se a composição tiver sido alterada depois.
- [ ] AC-24 — Realizar entrada manual ou sangria.
- [ ] AC-25 — Fechar caixa.
- [ ] AC-26 — Visualizar dashboard com estoque zerado e abaixo do mínimo.
...
| PDV | 8–9 | venda, PaymentMethod configurável, pagamentos e baixa direta/por composição |
| Pós-venda | 10 | consulta e cancelamento baseado nos movimentos originais |
| Integração | 11 | dashboard, alertas de estoque e jornada completa |
...
## Sprint 8 — Venda, Itens e Pagamentos Configuráveis

**Objetivo:** estruturar domínio de venda, PaymentMethod e PDV.

- [ ] Criar app `apps/sales`.
- [ ] Criar `Sale`.
- [ ] Criar `SaleItem`.
- [ ] Criar `PaymentMethod` por Company.
- [ ] Criar `Payment` referenciando `PaymentMethod`.
- [ ] Criar migrations.
- [ ] Criar seed/service idempotente dos métodos padrão: cash, pix, credit_card, debit_card.
- [ ] Garantir métodos padrão para Companies existentes sem duplicação.
- [ ] Permitir ativar/inativar métodos padrão.
- [ ] Preservar pagamentos históricos quando método for inativado.
- [ ] Criar endpoints de configuração de formas de pagamento.
- [ ] Criar tela de configuração de formas de pagamento.
- [ ] Definir status de venda.
- [ ] Gerar número da venda no backend.
- [ ] Preservar preço histórico do item.
- [ ] Criar service de cálculo.
- [ ] Criar tela de PDV.
- [ ] Buscar somente produtos vendáveis por nome/código/barcode.
- [ ] Adicionar/remover item no frontend.
- [ ] Alterar quantidade.
- [ ] Exibir subtotal provisório na UI.
- [ ] Listar no PDV somente PaymentMethods ativos.
- [ ] Selecionar forma de pagamento.
- [ ] Não tratar valor, comportamento de estoque, composição ou PaymentMethod enviado pela UI como fonte definitiva.
- [ ] Proteger configuração e venda pelas permissões configuráveis.

**Entrega:** PDV pronto para finalização segura com formas de pagamento configuráveis.

---

## Sprint 9 — Finalização Transacional da Venda

**Objetivo:** completar venda de ponta a ponta respeitando `direct`, `none` e `components`.

- [ ] Implementar service de finalização em `transaction.atomic()`.
- [ ] Validar usuário/permissão efetiva.
- [ ] Validar Company/Branch.
- [ ] Validar CashSession aberta.
- [ ] Carregar produtos no backend.
- [ ] Validar `is_sellable`.
- [ ] Carregar `inventory_behavior` do backend.
- [ ] Resolver necessidade real de estoque para cada item.
- [ ] `direct`: consumir o próprio Product.
- [ ] `none`: não gerar necessidade de estoque.
- [ ] `components`: expandir ProductComponent e consumir componentes.
- [ ] Consolidar necessidades repetidas do mesmo Product/Branch.
- [ ] Recalcular preços.
- [ ] Recalcular subtotais.
- [ ] Recalcular total.
- [ ] Validar PaymentMethod existente, ativo e pertencente à Company.
- [ ] Validar pagamentos.
- [ ] Bloquear somente os registros de estoque efetivamente necessários.
- [ ] Revalidar saldo dentro da transação.
- [ ] Criar itens históricos.
- [ ] Registrar pagamentos.
- [ ] Baixar estoque real.
- [ ] Criar `StockMovement(type='sale')` para cada produto físico consumido e vinculá-lo à Sale.
- [ ] Não criar StockMovement para produto `none`.
- [ ] Não baixar o estoque do produto pai `components`.
...
- [ ] Validar que venda `direct` com estoque insuficiente é rejeitada.
- [ ] Validar que venda `components` com qualquer componente insuficiente é rejeitada.
- [ ] Validar que produto `none` vende sem saldo de estoque.
- [ ] Validar que venda com caixa fechado é rejeitada.
- [ ] Validar que método de pagamento inativo é rejeitado mesmo se enviado manualmente à API.

**Entrega:** venda completa com pagamento configurável e baixa correta do estoque físico.
...
- [ ] Exibir forma de pagamento histórica mesmo se estiver inativa atualmente.
...
- [ ] Buscar `StockMovement(type='sale')` da venda original.
- [ ] Bloquear os estoques correspondentes durante devolução quando necessário.
- [ ] Devolver exatamente Products e quantidades dos movimentos originais.
- [ ] Criar `StockMovement(type='sale_cancellation')` inverso.
- [ ] Não recalcular devolução pela composição atual.
- [ ] Validar cancelamento de venda de produto `direct`.
- [ ] Validar cancelamento de produto `none` sem alteração de estoque.
- [ ] Validar cancelamento de produto `components` após alteração posterior da composição, devolvendo a composição histórica efetivamente consumida.
...
- [ ] Considerar insumos não vendáveis usados em composições nos alertas de estoque.
- [ ] Ignorar produto `none` em alertas de disponibilidade física.
- [ ] Últimas vendas.
- [ ] Respeitar Company/Branch autorizadas e permissões efetivas.
- [ ] Criar dashboard frontend.
- [ ] Revisar navegação por permissões, sem condições hardcoded por nome de perfil.
- [ ] Revisar feedbacks de loading/error/empty.
- [ ] Revisar responsividade.
- [ ] Percorrer fluxo Empresa → Perfil → Usuário → Produto → Estoque → Pagamento → Caixa → Venda → Cancelamento → Dashboard.
...
- [ ] Validar criação idempotente de perfis padrão.
- [ ] Validar criação/edição/inativação de perfil personalizado.
- [ ] Validar autorização por permissão, não por nome do perfil.
- [ ] Validar usuário com perfis diferentes em duas Companies quando cenário estiver disponível.
- [ ] Validar categoria.
- [ ] Validar produto `direct`.
- [ ] Validar produto `none`.
- [ ] Validar insumo `is_sellable=False`.
- [ ] Validar produto `components` e composição.
- [ ] Validar bloqueio de composição recursiva.
- [ ] Validar entrada de estoque.
- [ ] Validar estoque mínimo por Branch.
- [ ] Validar bloqueio de estoque negativo.
- [ ] Validar criação idempotente dos PaymentMethods padrão.
- [ ] Validar ativação/inativação de forma de pagamento.
- [ ] Validar abertura de caixa.
- [ ] Validar venda com produto `direct`.
- [ ] Validar venda com produto `none`.
- [ ] Validar venda com produto `components`.
- [ ] Validar pagamento com método ativo.
- [ ] Validar rejeição de método inativo.
- [ ] Validar baixa direta.
- [ ] Validar baixa de componentes.
- [ ] Validar ausência de baixa para `none`.
- [ ] Validar detalhe.
- [ ] Validar cancelamento.
- [ ] Validar devolução baseada nos movimentos originais.
- [ ] Alterar uma composição após venda e validar que o cancelamento continua devolvendo a composição histórica consumida.
- [ ] Validar entrada manual.
- [ ] Validar sangria.
- [ ] Validar fechamento.
- [ ] Validar dashboard com zerados e abaixo do mínimo.
- [ ] Executar build do frontend.
- [ ] Corrigir erros de TypeScript/JavaScript.
- [ ] Revisar rotas mortas.
- [ ] Revisar IDs hardcoded.
- [ ] Revisar nomes de perfil hardcoded em autorização.
- [ ] Revisar métodos de pagamento hardcoded fora do seed/configuração padrão.
- [ ] Revisar uso indevido de float.
- [ ] Revisar autorização de Company/Branch.
- [ ] Confirmar que módulos avançados fora do MVP não foram implementados.

**Entrega:** evidência técnica de que todos os critérios de aceite atualizados funcionam.
...
- [ ] Mesmo User pode possuir perfil diferente por Company.
...
- [ ] Caixa fecha.
- [ ] Dashboard mostra estoque zerado.
- [ ] Dashboard mostra estoque abaixo do mínimo.
...
- [ ] Nenhum módulo avançado fora do MVP foi implementado prematuramente.
...
6. autorização configurável por empresa;
7. comportamento de estoque explícito e previsível;
8. pagamentos configuráveis sem perder histórico;
9. frontend rápido e claro;
10. arquitetura capaz de evoluir sem reescrever o núcleo.
...
**Empresa → Filial → Perfil/Permissões → Usuário → Categoria → Produto/Composição → Estoque/Mínimo → Formas de Pagamento → Caixa → Venda → Pagamento → Baixa Direta/Componentes → Consulta/Cancelamento Histórico → Fechamento de Caixa → Dashboard.**
