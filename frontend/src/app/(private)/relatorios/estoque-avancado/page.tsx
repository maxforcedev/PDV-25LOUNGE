...
import { countStatusLabels, divergenceStatusLabels, inventoryDecimalSign, lossReasonLabels, normalizeQuantityGroups, physicalQuantityDisplay, resolutionTypeLabels, transferStatusLabels } from "@/lib/inventory";
...
function AdvancedReport() {
  ...
  const countRows: DetailRow[] = details?.event_rows.inventory_counts.map((row) => { const quantity = advancedQuantity(row, "difference_quantity", ["difference_content", "content_quantity"], "difference"); return { key: `count-${row.inventory_count_item}`, eventAt: row.event_at, title: `Diferença · ${row.product_name}`, resource: `Inventário ${row.inventory_count} · item ${row.inventory_count_item}`, detail: `${countStatusLabels[row.status]} · produto #${row.product}`, quantityDisplay: inventoryDecimalSign(row.difference_content ?? row.difference_quantity) === 1 ? `+${quantity}` : quantity, movementIds: row.movement_ids }; }) ?? [];
