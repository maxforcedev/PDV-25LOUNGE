  ...
  return <>
    ...
    <div className="space-y-4 p-4 sm:p-6 lg:p-8">
      ...
      <section className="card overflow-hidden"><div className="card-header"><div><h2 className="text-sm font-bold">Parcelas de compras</h2><p className="mt-1 text-[11px] text-muted">{items.length} {items.length === 1 ? "parcela" : "parcelas"}</p></div><Banknote className="size-5 text-muted" /></div>{loading ? <TableLoading columns={7} /> : items.length ? <>
        <div className="divide-y divide-subtle md:hidden">{items.map((item) => <article key={item.id} className="space-y-3 p-4"><div className="flex justify-between gap-3"><div><Link href={`/compras/${item.purchase_order}`} className="font-bold text-link">{item.order_number}</Link><p className="mt-1 text-xs text-muted">{item.supplier_name} · parcela {item.installment_number}</p></div><PayableBadge status={item.status} /></div><div className="flex items-end justify-between"><div><span className="text-[10px] text-muted">Vencimento</span><strong className="block text-xs">{new Date(`${item.due_date}T12:00:00`).toLocaleDateString("pt-BR")}</strong></div><strong>{formatBRL(item.amount)}</strong></div>{item.status === "PENDING" && <div className="flex justify-end gap-2 border-t border-subtle pt-3"><Button variant="secondary" onClick={() => open(item, "cancel")} disabled={readOnly}><XCircle className="size-4" />Cancelar</Button><Button onClick={() => open(item, "pay")} disabled={readOnly}><Banknote className="size-4" />Pagar</Button></div>}</article>)}</div>
