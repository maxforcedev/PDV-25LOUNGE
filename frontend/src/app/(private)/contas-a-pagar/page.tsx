...
import { formatBRL, formatDate, formatDecimalBRL, formatEditableDecimal } from "@/lib/format";
...
function PurchasePayables() {
  ...
  function open(item: PayableInstallment, next: "pay" | "cancel") { if (readOnly) return; setSelected(item); setAction(next); setText(""); setPaymentMethod("PIX"); setPaidAmount(formatEditableDecimal(item.amount)); setPaidDate(new Date().toISOString().slice(0, 10)); setError(""); }
  ...
  return <>
    ...
    <div className="space-y-4 p-4 sm:p-6 lg:p-8">
      ...
      <section className="card overflow-hidden"><div className="card-header"><div><h2 className="text-sm font-bold">Parcelas de compras</h2><p className="mt-1 text-[11px] text-muted">{items.length} {items.length === 1 ? "parcela" : "parcelas"}</p></div><Banknote className="size-5 text-muted" /></div>{loading ? <TableLoading columns={7} /> : items.length ? <>
        <div className="divide-y divide-subtle md:hidden">{items.map((item) => <article key={item.id} className="space-y-3 p-4"><div className="flex justify-between gap-3"><div><Link href={`/compras/${item.purchase_order}?origin=payables`} className="font-bold text-link">{item.order_number}</Link><p className="mt-1 text-xs text-muted">{item.supplier_name} · parcela {item.installment_number}</p></div><PayableBadge status={item.status} /></div><div className="flex items-end justify-between"><div><span className="text-[10px] text-muted">Vencimento</span><strong className="block text-xs">{new Date(`${item.due_date}T12:00:00`).toLocaleDateString("pt-BR")}</strong></div><strong>{formatDecimalBRL(item.amount)}</strong></div>{item.status === "PENDING" && <div className="flex justify-end gap-2 border-t border-subtle pt-3"><Button variant="secondary" onClick={() => open(item, "cancel")} disabled={readOnly}><XCircle className="size-4" />Cancelar</Button><Button onClick={() => open(item, "pay")} disabled={readOnly}><Banknote className="size-4" />Pagar</Button></div>}</article>)}</div>
        <div className="table-wrap hidden md:block"><table className="data-table min-w-225"><thead><tr><th>Compra</th><th>Fornecedor</th><th>Parcela</th><th>Vencimento</th><th>Valor</th><th>Status</th><th>Baixa</th><th className="text-right">Ações</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><Link href={`/compras/${item.purchase_order}?origin=payables`} className="font-bold text-link">{item.order_number}</Link></td><td>{item.supplier_name}</td><td>{item.installment_number}</td><td>{new Date(`${item.due_date}T12:00:00`).toLocaleDateString("pt-BR")}</td><td className="font-bold">{formatDecimalBRL(item.amount)}</td><td><PayableBadge status={item.status} /></td><td>{item.paid_at ? formatDate(item.paid_at) : item.cancelled_at ? formatDate(item.cancelled_at) : "-"}</td><td><div className="flex justify-end gap-1"><Link className="icon-button" href={`/compras/${item.purchase_order}?origin=payables`} title="Ver compra"><Eye className="size-4" /></Link>{item.status === "PENDING" && <><button className="icon-button" title="Registrar pagamento" onClick={() => open(item, "pay")} disabled={readOnly}><Banknote className="size-4" /></button><button className="icon-button" title="Cancelar parcela" onClick={() => open(item, "cancel")} disabled={readOnly}><XCircle className="size-4" /></button></>}</div></td></tr>)}</tbody></table></div>
      </> : <EmptyState title="Nenhuma parcela encontrada" description="Não há contas de compras para os filtros informados." />}</section>
    </div>
    <Modal open={!!action && !!selected} title={action === "pay" ? "Registrar pagamento manual" : "Cancelar parcela"} description={selected ? `${selected.order_number} · parcela ${selected.installment_number} · ${formatDecimalBRL(selected.amount)}` : ""} onClose={() => !acting && setAction(null)} size="md"><form onSubmit={submit}><div className="space-y-4 p-5">{error && <Alert message={error} />}{action === "pay" && <div className="grid gap-4 sm:grid-cols-2"><Field label="Forma efetivamente paga"><Select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)}><option>PIX</option><option>Transferência</option><option>Boleto</option><option>Dinheiro</option><option>Cartão</option><option>Outro</option></Select></Field><Field label="Data do pagamento"><Input required type="date" value={paidDate} onChange={(event) => setPaidDate(event.target.value)} /></Field><Field label="Valor pago"><Input required inputMode="decimal" value={paidAmount} onChange={(event) => setPaidAmount(event.target.value)} /></Field></div>}<Field label={action === "pay" ? "Observação" : "Motivo"} optional><Textarea value={text} onChange={(event) => setText(event.target.value)} disabled={acting} /></Field>{action === "pay" && <p className="text-xs text-warning-strong">A baixa, forma efetiva, data e valor ficam registrados na parcela de fornecedor.</p>}</div><div className="flex justify-end gap-2 border-t border-subtle p-4"><Button type="button" variant="secondary" onClick={() => setAction(null)} disabled={acting}>Voltar</Button><Button type="submit" variant={action === "cancel" ? "danger" : "primary"} loading={acting}>{action === "pay" ? "Confirmar pagamento" : "Cancelar parcela"}</Button></div></form></Modal>
  </>;
}
