...
import type { Paginated, AuditLog, User, UserPayload, UserType } from "@/types";
...
function UsersAdministration() {
    ...
    if (saved)
      setSuccess(
        saved === "created"
          ...
          : saved === "archived"
            ? "Usuário arquivado. O histórico foi preservado."
            : saved === "restored"
  ...
  useEffect(() => {
    ...
    let active = true;
    http
      .get<User>(`users/${userId}/?company=${companyId}`)
      .then((target) => {
        if (!active || contextRef.current !== contextKey) return;
        if (target.membership?.company_id !== companyId)
  ...
  useEffect(() => {
    ...
    http
      .get<Paginated<AuditLog>>(
        `audit-logs/?company=${currentCompany.id}&object_type=User&search=${editing.id}`,
      )
      .then((result) =>
        setHistory(
          result.results.filter((log) => log.object_id === String(editing.id)),
        ),
      )
      .catch(() => setHistory([]))
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (editing)
        await http.patch(
          `users/${editing.id}/?company=${currentCompany?.id}`,
          payload,
        );
      else
        await http.post<User>(`users/?company=${currentCompany?.id}`, payload);
      if (isDetail) {
  ...
  async function changeStatus() {
    ...
    try {
      const updated = await http.post<User>(
        `users/${confirming.id}/${action}/?company=${currentCompany?.id}`,
      );
      setConfirming(null);
  ...
  async function archiveUser() {
    ...
    try {
      await http.post(
        `users/${archiving.id}/archive/?company=${currentCompany?.id}`,
      );
      setArchiving(null);
      if (isDetail) {
        router.push("/usuarios?saved=archived");
        return;
      }
  ...
  return (
    <>
                      ...
                      {data.results.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td>
                            <StatusBadge
                              active={
                                !!item.membership?.is_active && item.is_active
                              }
                            />
                          </td>
                          <td>
      ...
      <UserEditorFrame
        ...
        description={
          editing
            ? `${typeLabel(editing.user_type)} · ${editing.membership?.is_active ? "Ativo" : "Inativo"}`
            : "Cadastre os dados principais da pessoa."
        }
        onClose={() =>
          !saving && (isDetail ? router.push("/usuarios") : setOpen(false))
        }
      >
        {isDetail && editing && (
          <>
            <div className="flex flex-wrap gap-2 border-b border-subtle p-4 sm:px-6">
              ...
              {canChange && (
                <Button type="submit" form="user-editor-form" loading={saving}>
                  Salvar usuário
                </Button>
              )}
              {canStatus && editing.id !== actor?.id && (
                <Button
                  type="button"
                  variant="secondary"
                  onClick={() => setConfirming(editing)}
                >
                  {editing.membership?.is_active ? "Inativar" : "Ativar"}
                </Button>
              )}
            </div>
            <div
            ...
            >
              {editorTabs.map(([value, label]) => (
                <button
                  ...
                  className={`shrink-0 border-b-2 px-4 py-3 text-xs font-semibold transition ${editorTab === value ? "border-primary text-primary" : "border-transparent text-muted hover:text-fg"}`}
                >
                  {label}
                </button>
              ))}
        ...
        <form id="user-editor-form" onSubmit={submit}>
          <fieldset
            ...
            className="p-5 disabled:opacity-75 sm:p-6"
          >
            {error && (
              <div className="mb-5">
                <Alert message={error} />
            ...
            {!editing ? (
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <Field
                  label="Sobrenome"
                  error={fieldError(fields, "last_name")}
                >
                  <Input
                    required
                ...
                <label className="flex items-center gap-3 rounded-lg border border-subtle p-4 text-xs font-semibold">
                  ...
                  <span>
                    ...
                    <small className="font-normal text-muted">
                      Controla somente a autenticação, sem remover vínculos
                      operacionais.
                    </small>
                  </span>
                </label>
                ...
                <div className="space-y-3 sm:col-span-2">
                  ...
                  {branches
                    ...
                    .map((branch) => {
                      const access = form.company_accesses[0];
                      const branchAccess = access?.branch_accesses.find(
                        (item) => item.branch_id === branch.id,
                      );
                      return (
                        <div
                          key={branch.id}
                          className="grid gap-2 rounded-lg border border-subtle p-3 sm:grid-cols-[1fr_14rem]"
                        >
                          <label className="flex items-center gap-2">
                            <input
                              ...
                              onChange={() =>
                                currentCompany &&
                                toggleBranch(currentCompany.id, branch.id)
                              }
                            />
                            <span>
              ...
              <>
                  ...
                  <div className="grid gap-5 sm:grid-cols-2">
                    <div className="flex items-center gap-4 sm:col-span-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted">
                          Identificação e endereço global do usuário.
                        </p>
                      </div>
                    </div>
                    <Field label="Nome">
                    ...
                    <Field label="Aniversário" optional>
                      <Input
                        type="date"
                        value={form.birth_date || ""}
                        onChange={(event) =>
                          update("birth_date", event.target.value || null)
                        }
                      />
                    </Field>
                    <Field label="CEP" optional>
                      <Input
                        value={form.zip_code}
                        onChange={(event) =>
                          update("zip_code", event.target.value)
                        }
                      />
                    </Field>
                    ...
                    <Field label="Número" optional>
                      <Input
                        ...
                        onChange={(event) =>
                          update("address_number", event.target.value)
                        }
                      />
                    </Field>
                    <Field label="Complemento" optional>
                      <Input
                        ...
                        onChange={(event) =>
                          update("address_complement", event.target.value)
                        }
                      />
                    </Field>
                    ...
                    <div className="flex items-center justify-between rounded-lg border border-subtle p-4">
                      <div>
                        <strong className="text-xs">
                          Estado do acesso nesta empresa
                        </strong>
                        <p className="mt-1 text-xs text-muted">
                          A credencial global permanece preservada em outras
                          empresas.
                        </p>
                      </div>
                ...
                {editorTab === "history" && (
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted">
                      Eventos auditáveis deste usuário na empresa atual.
                    </p>
                    {historyLoading ? (
      ...
      >
        <div className="space-y-4 p-5">
          <div className="rounded-lg border border-subtle bg-surface-soft p-4 text-sm">
            ...
            <span className="block text-muted">
              Excluído em:{" "}
              {restoreConflict ? formatDate(restoreConflict.archivedAt) : ""}
            </span>
          </div>
          <p className="text-sm text-muted">
