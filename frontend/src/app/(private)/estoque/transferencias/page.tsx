...
import { inventoryTone, transferStatusLabels } from "@/lib/inventory";
    ...
    <div className="space-y-4 p-4 sm:p-6 lg:p-8">
      ...
      <section className="card overflow-hidden">
        ...
        {loading ? <TableLoading columns={6} /> : items.length ? <>
          <div className="divide-y divide-subtle md:hidden">{items.map((transfer) => <article key={transfer.id} className="space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div><Link href={`/estoque/transferencias/${transfer.id}`} className="font-bold text-link">{transfer.id.slice(0, 8).toUpperCase()}</Link><p className="mt-1 text-xs text-muted">{transfer.origin_branch_name} → {transfer.destination_branch_name}</p></div><span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${inventoryTone(transfer.status)}`}>{transferStatusLabels[transfer.status]}</span></div><dl className="grid grid-cols-2 gap-2 text-xs"><div><dt className="text-muted">Criada em</dt><dd>{formatDate(transfer.created_at)}</dd></div><div><dt className="text-muted">Itens</dt><dd>{transfer.items.length}</dd></div><div><dt className="text-muted">Solicitado</dt><dd>{formatQuantity(String(transfer.items.reduce((sum, item) => sum + Number(item.requested_quantity), 0)))}</dd></div><div><dt className="text-muted">Pendente</dt><dd>{formatQuantity(String(transfer.items.reduce((sum, item) => sum + Number(item.pending_quantity || 0), 0)))}</dd></div></dl></article>)}</div>
