...
function TransferDetail() {
  ...
  return <>
    ...
    <Modal open={cancelOpen} title="Cancelar rascunho" description="Somente transferências ainda não despachadas podem ser canceladas." onClose={() => !saving && setCancelOpen(false)} size="md"><form onSubmit={cancel}><div className="space-y-4 p-5">{error && <Alert message={error} />}<Field label="Motivo" optional error={fieldError(fields, "reason")}><Textarea value={reason} onChange={(event) => setReason(event.target.value)} /></Field></div><div className="flex justify-end gap-2 border-t border-subtle px-5 py-4"><Button type="button" variant="secondary" onClick={() => setCancelOpen(false)} disabled={saving}>Voltar</Button><Button type="submit" variant="danger" loading={saving}>Cancelar transferência</Button></div></form></Modal>
  </>;
}
