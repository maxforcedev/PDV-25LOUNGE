...
function Movements() {
  ...
  return <><PageHeader title="Movimentações de estoque" description={`Histórico imutável de ${currentBranch?.name || "filial atual"}.`} action={<Link href="/estoque" className="btn btn-secondary"><ArrowLeft className="size-4" />Voltar ao estoque</Link>} /><div className="space-y-4 p-4 sm:p-6 lg:p-8">
    ...
    <section className="card overflow-hidden"><div className="card-header"><div><h2 className="text-sm font-bold">Histórico</h2><p className="mt-1 text-[11px] text-slate-500">Movimento e transição de saldo</p></div><History className="size-5 text-slate-300" /></div>{loading ? <TableLoading /> : data?.results.length ? <><div className="table-wrap"><table className="data-table"><thead><tr><th>Data</th><th>Produto</th><th>Tipo</th><th>Quantidade</th><th>Transição de saldo</th><th>Responsável</th><th>Origem / Detalhes</th></tr></thead><tbody>{data.results.map((movement) => { const amount = signed(movement); const positive = amount.startsWith("+"); const automatic = ["sale", "sale_cancellation", "consumption", "consumption_cancellation"].includes(movement.type); return <tr key={movement.id}><td className="whitespace-nowrap">{formatDate(movement.created_at)}</td><td><strong className="block">{movement.product_name}</strong><span className="text-[11px] text-slate-400">{movement.internal_code}</span></td><td><span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${tones[movement.type] || tones.cancellation}`}>{labels[movement.type] || movement.type}</span></td><td className={`font-bold ${positive ? "text-emerald-700" : "text-red-700"}`}>{amount} {movement.unit.toUpperCase()}</td><td>{formatQuantity(movement.previous_quantity)} → {formatQuantity(movement.final_quantity)}</td><td>{movement.user_name}</td><td className="min-w-48">{automatic && movement.sale_number ? <Link className="font-bold text-primary" href={`${movement.sale_operation_type === "consumption" ? "/consumacoes" : "/vendas"}/${movement.sale}`}>{movement.sale_number}</Link> : <span className="text-slate-500">{movement.reason || "-"}</span>}{automatic && movement.reason && <span className="block text-[10px] text-slate-400">{movement.reason}</span>}</td></tr>; })}</tbody></table></div><Pagination count={data.count} next={data.next} previous={data.previous} onPage={load} /></> : <EmptyState title="Nenhuma movimentação" description="Ajuste os filtros ou registre uma operação de estoque." />}</section>
  </div></>;
}
