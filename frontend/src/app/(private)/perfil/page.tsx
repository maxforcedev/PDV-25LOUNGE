...
export default function ProfilePage() {
  ...
  return (
    <>
      <PageHeader title="Meu perfil" description="Consulte seus acessos e mantenha seus dados pessoais atualizados." />
                ...
                return (
                  <article key={company.id} className="p-5 sm:p-6">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <h3 className="text-sm font-bold text-dark">{company.trade_name}</h3>
                        {company.is_owner && (
                    ...
                    {branches.length ? (
                      <div className="mt-4 grid gap-2 sm:grid-cols-2">
                        {branches.map((branch) => (
                          <div key={branch.id} className="flex items-center gap-2.5 rounded-md border border-slate-100 bg-slate-50/70 px-3 py-2.5 text-xs text-slate-600">
                            <GitBranch className="size-3.5 shrink-0 text-primary" />
                            <span>
                              <strong className="block font-medium">{branch.name}</strong>
                              <small className="text-[10px] text-slate-400">Perfil: {user.is_superuser ? "Superusuário" : branch.access_profile?.name || "Sem perfil"}</small>
                            </span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="mt-4 rounded-md border border-dashed border-slate-200 p-3 text-center text-[11px] text-slate-400">Nenhuma filial vinculada nesta empresa.</p>
                    )}
                  </article>
                );
              })}
