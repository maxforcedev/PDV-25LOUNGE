...
function Kpi({
  ...
  return (
    <Link
      href={href}
      className="card group p-5 transition hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-focus/25"
    >
...
function PaymentChart({
  ...
  const title = scope === "operational" ? "Formas de pagamento operacionais" : "Formas de pagamento das vendas";
  ...
  return (
    <section className="card overflow-hidden">
      ...
      {rows.length ? (
        <div className="p-5" aria-label="Distribuição por forma de pagamento">
          <div className="mb-5 flex h-5 overflow-hidden rounded-full bg-surface-muted">
            {rows.map((row, index) => (
              <Link
                key={`${row.code}-${index}`}
                href={`${href}&payment_method_code=${encodeURIComponent(row.code)}`}
                title={`${row.name}: ${formatBRL(row.amount)}`}
                aria-label={`${row.name}: ${formatBRL(row.amount)}`}
                className={`${colors[index % colors.length]} min-w-1 transition hover:brightness-110 focus-visible:z-10 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-focus`}
                style={{
                  width: `${total ? (Number(row.amount) * 100) / total : 0}%`,
                }}
              />
            ))}
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {rows.map((row, index) => (
              <Link
                key={`${row.name}-${index}`}
                href={`${href}&payment_method_code=${encodeURIComponent(row.code)}`}
                className="flex items-center justify-between gap-3 rounded text-xs focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-focus/25"
              >
                <span className="flex min-w-0 items-center gap-2">
                  <i
                    className={`size-2.5 shrink-0 rounded-full ${colors[index % colors.length]}`}
                  />
...
function WeeklyComparison({
  ...
  const max = Math.max(
    ...points.flatMap((point) => [
      Number(point.current?.revenue || 0),
      Number(point.previous?.revenue || 0),
    ]),
    0,
  );
      ...
      {hasData ? (
        <div className="p-5">
          ...
          <div
            ...
            aria-label="Faturamento atual comparado ao período anterior"
          >
                ...
                <div className="flex h-42 items-end justify-center gap-1">
                  {(
                    ...
                    series.row ? (
                      <Link
                        key={series.label}
                        href={dayHref(series.row.date)}
                        aria-label={`${series.label}, ${series.row.date}: ${formatBRL(series.row.revenue)}`}
                        className={`group relative w-3 rounded-t ${series.tone} transition hover:brightness-110 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-focus`}
                        style={{
                          height: `${max ? Math.max(2, (Number(series.row.revenue) * 100) / max) : 0}%`,
                        }}
                      >
                        <span className="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max -translate-x-1/2 rounded bg-chart-tooltip px-2 py-1 text-[10px] text-chart-tooltip-fg opacity-0 shadow-lg transition group-hover:opacity-100 group-focus-visible:opacity-100">
                          {series.label}: {formatBRL(series.row.revenue)}
                        </span>
                      </Link>
                    ) : (
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <th>Faturamento</th>
                <th>Ticket</th>
                {seller && rows.some((row) => row.commission !== undefined) && (
                  <th>Comissão</th>
                )}
              </tr>
            </thead>
                ...
                <tr key={row.user.id}>
                  <td>
                    <Link
                      ...
                      href={`${href}&${seller ? "seller" : "operator"}=${row.user.id}`}
                    >
                      {row.user.name}
                    </Link>
                  </td>
                  <td>{row.count}</td>
                  <td>{formatBRL(row.effective_revenue)}</td>
                  <td>{formatBRL(row.average)}</td>
                  {seller &&
                    rows.some((item) => item.commission !== undefined) && (
                      <td>
...
function DashboardPage() {
  ...
  const heatMax = Math.max(
    ...(sales?.heatmap || []).map((row) => Number(row.revenue)),
    0,
  );
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            {[
              ...
              <button
                ...
                onClick={() => setDraftPeriod(value as PeriodValue)}
              >
                {String(label)}
              </button>
            ))}
            ...
            <Button
              className="ml-auto"
              onClick={() => void load(draftPeriod, draftCategory)}
            >
              Aplicar
            </Button>
          </div>
              ...
              {sales && (
                <>
                  <Kpi
                    ...
                    value={formatBRL(sales.effective_revenue)}
                    note={`${sales.count} vendas comerciais, sem taxa`}
                    icon={TrendingUp}
                    href={report("vendas")}
                  />
                  <Kpi
                    label="Taxa de serviço"
                    value={formatBRL(sales.service_fee)}
                    note="Parcela cobrada além do faturamento efetivo"
                    icon={ReceiptText}
                    href={report("vendas")}
                    tone="success"
                  />
                  <Kpi
                    label="Total recebido em vendas"
                    value={formatBRL(sales.total_received_sales)}
                    note={`Total cobrado ${formatBRL(sales.customer_total)}`}
                    icon={ReceiptText}
                    href={report("recebimentos")}
                  />
                  {sales.total_received_operational && (
                    <Kpi
                      label="Total recebido operacional"
                      value={formatBRL(sales.total_received_operational)}
                      note="Vendas + consumações cobradas, líquido de reversões"
                      icon={TrendingUp}
                      href={report("recebimentos")}
                      tone="success"
                    />
                  ...
                  <Kpi
                    ...
                    note="Receita efetiva por venda"
                    icon={ReceiptText}
                    href={report("vendas")}
                  />
                  <Kpi
                  ...
                  <Kpi
                    ...
                    icon={ReceiptText}
                    tone="warning"
                    href={report("descontos")}
                  />
                </>
              )}
              {data.consumptions && (
                  <Kpi
                    label="Consumação & Cortesias"
                    value={formatBRL(data.consumptions.charged)}
                  note={`${data.consumptions.count} pedidos · referência ${formatBRL(data.consumptions.reference)} · benefício ${formatBRL(data.consumptions.subsidy)}`}
                  icon={ShoppingBasket}
                  href={report("consumacoes")}
                />
              )}
                ...
                <section className="card p-5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <Kpi
                      ...
                      icon={Boxes}
                      tone="warning"
                      href="/estoque?state=below_minimum"
                    />
                    <Kpi
                ...
                {data.operational_result && (
                  <div className="mb-4 rounded-lg border border-dashed border-primary/30 p-4">
                    ...
                    <strong className="mt-2 block text-2xl">
                      {formatBRL(data.operational_result.result)}
                    </strong>
                    <small className="text-slate-500">
                      Recebimento operacional {formatBRL(data.operational_result.operational_received)} · Margem {data.operational_result.margin}%
                    </small>
                  </div>
                )}
              ...
              <>
                <section className="card overflow-hidden">
                  <div className="card-header">
                    <div>
                      ...
                      <p className="mt-1 text-[11px] text-slate-500">
                        Faturamento efetivo, vendas e ticket no tooltip.
                      </p>
                    </div>
                  </div>
                            ...
                            {Array.from({ length: 24 }, (_, hour) => {
                              ...
                              return (
                                <Link
                                  ...
                                  title={
                                    cell
                                      ? `${formatBRL(cell.revenue)} · ${cell.count} vendas · ticket ${formatBRL(cell.average)}`
                                      : "Sem vendas"
                                  }
                                  aria-label={
                                    cell
                                      ? `${day}, ${hour} horas: ${formatBRL(cell.revenue)}, ${cell.count} vendas`
                                      : `${day}, ${hour} horas: sem vendas`
                                  }
                                  className="group relative aspect-square rounded-sm border border-chart-1/20 focus-visible:z-10 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-focus"
                                  style={{
                ...
                <div className="grid gap-5 xl:grid-cols-2">
                  ...
                  <HorizontalBars
                    ...
                    href={report("atendentes")}
                    rows={sales.top_sellers.slice(0, 8).map((row) => ({
                      label: row.user.name,
                      value: Number(row.effective_revenue),
                      display: formatBRL(row.effective_revenue),
                      note: `${row.count} vendas`,
                      query: `&seller=${row.user.id}`,
                    }))}
                  />
                  <WeeklyComparison
                        ...
                        <tbody>
                          {sales.latest_sales.map((sale) => (
                            <tr key={sale.id}>
                              ...
                              <td>{formatBRL(sale.total_received_sales || sale.total)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
