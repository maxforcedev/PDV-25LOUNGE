...
function BranchPrices() {
  ...
  >({});
  const [drafts, setDraftValues] = useState<Record<number, string>>({});
  ...
  function setDrafts(update: SetStateAction<Record<number, string>>) {
    if (typeof update === "function") {
      setDraftValues(update);
      return;
    }
    setDraftValues((current) => {
      const next = { ...current, ...update };
      return Object.fromEntries(
        Object.entries(next).map(([product, value]) => [
          product,
  ...
  async function load() {
    ...
    try {
      ...
      const normalizedMatrix = {
        ...matrix,
        overrides: matrix.overrides.map((price) => ({
          ...price,
          sale_price: formatEditableDecimal(price.sale_price),
        })),
        products: matrix.products.map((product) => ({
          ...product,
          default_price: formatEditableDecimal(product.default_price),
          prices: Object.fromEntries(
            Object.entries(product.prices).map(([branch, price]) => [
              branch,
              price === null ? null : formatEditableDecimal(price),
            ]),
          ),
        })),
      };
      const byProduct = Object.fromEntries(
        normalizedMatrix.overrides.map((price) => [price.product, price]),
      );
      setComparison(normalizedMatrix);
      setOverrides(byProduct);
      setDrafts(
        Object.fromEntries(
          normalizedMatrix.products.map((product) => [
            product.id,
            product.availability[String(branchId)]
      ...
      if (branchRef.current === branchId)
        setError(
          caught instanceof ApiError
            ? caught.message
            : "Não foi possível carregar os preços por filial.",
        );
    } finally {
      if (branchRef.current === branchId) setLoading(false);
    }
  }
  ...
  async function save(productId: number) {
    ...
    setLineErrors({});
    try {
      await http.post<BulkPriceResponse>("branch-prices/bulk/", {
        branch: branchId,
        items,
      });
      if (branchRef.current !== branchId) return;
  ...
  async function saveAll() {
    ...
    const changed = comparison.products.filter(
      (product) =>
        product.availability[String(branchId)] &&
        drafts[product.id] !== undefined &&
        !decimalEquals(
          drafts[product.id],
          overrides[product.id]?.sale_price || product.default_price,
        ),
    );
    if (!changed.length) {
      setSuccess("Nenhuma alteração pendente.");
      return;
    }
    const items = changed.map((product) => ({
      product: product.id,
      sale_price: drafts[product.id],
    }));
    setSavingAll(true);
    setError("");
    setSuccess("");
    setLineErrors({});
    try {
      const result = await http.post<BulkPriceResponse>("branch-prices/bulk/", {
        branch: branchId,
        items,
      });
      if (branchRef.current !== branchId) return;
  ...
  async function useDefault(productId: number) {
    ...
    try {
      ...
      setError(
        caught instanceof ApiError
          ? caught.message
          : "Não foi possível restaurar o preço padrão.",
      );
    } finally {
      setSaving(null);
    }
  ...
  const products =
    comparison?.products.filter((product) => {
      const term = search.trim().toLocaleLowerCase("pt-BR");
      return (
        !term ||
        product.name.toLocaleLowerCase("pt-BR").includes(term) ||
        product.internal_code.toLocaleLowerCase("pt-BR").includes(term)
      );
    }) || [];
  ...
  return (
    <>
      <PageHeader
        ...
        description="Compare e ajuste preços de produtos por filial."
        action={
      ...
      <div className="space-y-4 p-4 sm:p-6 lg:p-8">
        ...
        {!!Object.keys(lineErrors).length && (
          <div role="alert" className="card border-danger/30 p-4">
            ...
            {Object.entries(lineErrors).map(([productId, fields]) => (
              <p key={productId} className="mt-2 text-xs text-danger-strong">
        ...
        <div className="card grid gap-4 p-4 lg:grid-cols-[1fr_auto] lg:items-center">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="rounded-lg bg-primary/5 px-4 py-2 text-xs text-slate-600">
              <strong className="text-primary">Filial editável:</strong>{" "}
              {currentBranch?.name || "nenhuma"}
            </div>
            <Button
        ...
        <section className="card overflow-hidden">
          <div className="card-header">
            <div>
              <h2 className="text-sm font-bold">Comparativo de preços</h2>
              {/* <p className="mt-1 text-[11px] text-slate-500">“Preço padrão” indica que não existe substituição para a filial. “Preço da filial” indica override ativo.</p> */}
            </div>
            <DollarSign className="size-5 text-slate-300" />
          </div>
          {loading ? (
            <TableLoading columns={(comparison?.branches.length || 1) + 3} />
          ) : products.length ? (
                ...
                <tbody>
                  {products.map((product) => (
                    <tr key={product.id}>
                      ...
                      {comparison?.branches.map((branch) => {
                        const specific = product.prices[String(branch.id)];
                        const available = product.availability[String(branch.id)];
                        ...
                        const changed =
                          ...
                          !decimalEquals(
                            drafts[product.id],
                            overrides[product.id]?.sale_price ||
                          ...
                          <td key={branch.id}>
                            {!available ? (
                              ...
                              <div className="min-w-36">
                                <MoneyInput
                                  aria-label={`Preço de ${product.name} em ${branch.name}`}
                                  value={
                                    drafts[product.id] || product.default_price
                                  }
                                  onValueChange={(value) =>
                                    setDrafts((current) => ({
                                      ...current,
                                ...
                                />
                                <span
                                  className={`mt-1 block text-[10px] ${changed ? "font-semibold text-amber-600" : specific === null ? "text-slate-400" : "font-semibold text-primary"}`}
                                >
                                  {changed
                                    ? "Alterado"
                                    : specific === null
                              ...
                              <div>
                                <strong className="text-xs">
                                  {formatBRL(specific || product.default_price)}
                                </strong>
                      ...
                      <td>
                        <div className="flex justify-end gap-1">
                          <Button
                            ...
                            disabled={
                              !canChange || saving !== null || savingAll
                              || !product.availability[String(currentBranch?.id || "")]
                            }
                          ...
                          <button
                            ...
                            onClick={() => void useDefault(product.id)}
                          >
...
export default function BranchPricesPage() {
  return (
    <AdminGuard
      requiredPermissions={[
        permissions.viewBranchPrice,
        permissions.viewCompanyBranchPrice,
