...
import {
  ProductV26Sections,
  type ProductV26Actions,
  type ProductV26Tab,
} from "@/components/product-v26-sections";
import {
  Alert,
  Button,
  ConfirmDialog,
...
const blank = (company = 0, includeCost = false): ProductForm => ({
  company,
  category: 0,
  name: "",
  description: "",
...
function scaled(value: string, places: number) {
  ...
  const digits = `${whole.replace(/\D/g, "") || "0"}${fraction.replace(/\D/g, "").padEnd(places, "0").slice(0, places)}`;
...
function componentSuggestion(
  ...
  candidates: Product[],
  field: "cost" | "sale_price",
) {
  const thousandths = components.reduce((total, component) => {
    const candidate = candidates.find(
      (item) => item.id === component.component_product,
    );
    const value = candidate?.[field];
    if (!value) return total;
    return total + scaled(value, 2) * scaled(component.quantity || "0", 3);
  }, BigInt(0));
  return money((thousandths + BigInt(500)) / BigInt(1000));
}
function displayComponent(component: ProductComponent, candidates: Product[]) {
  if (component.quantity_display) return component.quantity_display;
  const candidate = candidates.find(
    (item) => item.id === component.component_product,
  );
...
const productTabs = [
  ["data", "Dados"],
  ["suppliers-stock", "Fornecedores e estoque"],
  ["prices-modifiers", "Preços e modificadores"],
  ["production", "Produção"],
  ["history", "Histórico"],
] as const satisfies ReadonlyArray<[ProductV26Tab, string]>;
...
function Products() {
  ...
  const canConfigureFraction = hasPermission(
    permissions.configureProductFraction,
  );
  const canConfigureDestinations = hasPermission(
  ...
  const productActionsRef = useRef<ProductV26Actions | null>(null);
  const openedQuery = useRef("");
  const suggestionCost = canViewCosts
    ? componentSuggestion(components, candidates, "cost")
    : undefined;
  const suggestionSale = componentSuggestion(
    components,
    candidates,
    "sale_price",
  );
  ...
  useEffect(() => {
    ...
    http
      ...
      .then((product) => {
        if (product.company !== currentCompany.id)
          throw new ApiError("Produto não encontrado.", 404);
        void show(product);
        if (new URLSearchParams(window.location.search).get("saved") === "1") {
          setSuccess("Produto salvo com sucesso.");
          window.history.replaceState(null, "", `/produtos/${productId}`);
        }
      })
      .catch((caught) =>
        setError(
  ...
  }, [id, isDetail, isNew, currentCompany?.id, detailRetry]);

  useEffect(() => {
    if (
      !currentCompany ||
      !currentBranch ||
      openedQuery.current === `${currentCompany.id}:${currentBranch.id}`
    )
      return;
    ...
    http
      ...
      .then((product) => {
        if (
          ...
          product.company === currentCompany.id
        )
          show(product);
      })
      .catch((caught) => {
        if (contextRef.current === context)
          setError(
            caught instanceof ApiError
              ? caught.message
              : "Não foi possível abrir o produto informado.",
          );
      });
  }, [currentCompany?.id, currentBranch?.id]);
      ...
      setForm(
        detail
          ? {
              ...
              internal_code: detail.internal_code,
              sku: detail.sku || "",
      ...
      if (detail?.inventory_behavior === "components") {
        const details = await Promise.all(
          candidates.map((candidate) =>
            http
              .get<Product>(`products/${candidate.id}/`)
              .catch(() => candidate),
          ),
        );
        setFractionCandidates(
          details.filter((candidate) => !!candidate.fraction_config),
        );
      }
    } catch (caught) {
      setError(
  ...
  function updateComponent(
    index: number,
    key: "component_product" | "quantity",
    value: number | string,
  ) {
    ...
    setComponents((current) => {
      const next = current.map((item, itemIndex) =>
        itemIndex === index
          ? { ...item, [key]: value, quantity_display: "" }
          : item,
      );
      const sale = componentSuggestion(next, candidates, "sale_price");
      setForm((formValue) => ({
        ...formValue,
        ...(canViewCosts && !touched.current.cost
          ? { cost: componentSuggestion(next, candidates, "cost") }
          : {}),
        sale_price: touched.current.sale_price ? formValue.sale_price : sale,
      }));
      return next;
    });
  }
  ...
  function addFractionComponent() {
    if (canCompose)
      setFractionComponents((current) => [
        ...current,
        {
          component_product: 0,
          component_name: "",
  ...
  function updateFractionComponent(
    index: number,
    key: "component_product" | "content_quantity",
    value: number | string,
  ) {
    if (!canCompose) return;
    setFractionComponents((current) =>
        ...
        position === index
          ? {
              ...
              ...(key === "component_product"
                ? {
                    content_unit:
                      fractionCandidates.find(
                        (candidate) => candidate.id === Number(value),
                      )?.fraction_config?.content_unit || "ml",
                  }
                : {}),
            }
          : item,
  ...
  async function submit(event: React.FormEvent) {
    ...
    setSuccess("");
    try {
      const composition = components.map(({ component_product, quantity }) => ({
        component_product,
        quantity,
      }));
      const exactComposition = fractionComponents.map(
        ({ component_product, content_quantity }) => ({
          component_product,
          content_quantity: content_quantity.replace(",", "."),
        }),
      );
      if (form.inventory_behavior === "components" && !canCompose && !editing)
        throw new ApiError(
          "Você não possui permissão para configurar composição de produtos.",
          403,
        );
      if (editing) {
      ...
      setSuccess(
        editing
          ? editing.inventory_behavior === "components" && !canCompose
            ? "Produto atualizado. A composição permaneceu inalterada por falta de permissão."
  ...
  async function changeStatus() {
    ...
    try {
      await http.post(`products/${confirming.id}/${action}/`);
      setConfirming(null);
      setSuccess(`Produto ${action === "activate" ? "ativado" : "inativado"}.`);
      await load();
    } catch (caught) {
      setConfirming(null);
      setError(
  ...
  async function archiveProduct() {
    if (!archiving || !canStatus) return;
    setSaving(true);
    try {
      const action = archiving.archived_at ? "restore" : "archive";
      await http.post(`products/${archiving.id}/${action}/`);
      setArchiving(null);
      setSuccess(
        action === "archive"
          ? "Produto arquivado e removido da operação."
          : "Produto restaurado para a operação.",
      );
      await load();
    } catch (caught) {
      setArchiving(null);
      setError(
        caught instanceof ApiError
          ? caught.message
          : "Não foi possível alterar o arquivamento do produto.",
      );
    } finally {
      setSaving(false);
    }
  ...
  function openFilters() {
    setDraft({ category, lifecycle, behavior, sellable, favorite });
    setFiltersOpen(true);
  }
  function applyFilters() {
    ...
    setSellable(draft.sellable);
    setFavorite(draft.favorite);
    setFiltersOpen(false);
    void load(query(draft));
  }
  function clearFilters() {
    ...
    setFavorite("");
    setFiltersOpen(false);
    void load(query(emptyFilters));
  }

  if (isDetail && !isNew && error && !open)
    return (
      <main className="flex min-h-[50vh] items-center justify-center p-4">
        <section className="card w-full max-w-lg space-y-4 p-6 text-center">
          <Alert message="Não foi possível carregar os dados do produto." />
          <div className="flex justify-center gap-2">
            <Button variant="secondary" onClick={() => setDetailRetry((value) => value + 1)}>
              Tentar novamente
            </Button>
            <Link href="/produtos" className="btn btn-secondary">Voltar para produtos</Link>
          </div>
        </section>
      </main>
    );

  if (isDetail && !isNew && !open)
    return (
      <main className="flex min-h-[50vh] items-center justify-center p-4">
        <TableLoading />
      </main>
  ...
  return (
    <>
      {!isDetail && (
        <PageHeader
          title="Produtos"
          description={`Catálogo, preços e composição de ${currentCompany?.trade_name || "sua empresa"}.`}
          action={
            <div className="flex flex-wrap gap-2">
              {user?.is_superuser && (
                <Link href="/produtos/lote" className="btn btn-secondary">
      ...
      {!isDetail && (
        <div className="space-y-4 p-4 sm:p-6 lg:p-8">
          ...
          <form
            className="card relative flex gap-2 p-4"
            onSubmit={(event) => {
              event.preventDefault();
              void load();
            }}
          >
            ...
            <Button type="button" variant="secondary" onClick={openFilters}>
              ...
              {Object.values({
                ...
                ? ` (${Object.values({ category, lifecycle, behavior, sellable, favorite }).filter(Boolean).length})`
                : ""}
            </Button>
            {filtersOpen && (
              <>
                ...
                >
                  ...
                  <div className="space-y-3">
                    ...
                    <Field label="Venda">
                      ...
                      >
                        <option value="">Venda e insumos</option>
                        <option value="true">Vendável</option>
                        <option value="false">Insumo</option>
                      </Select>
                    </Field>
          ...
          <section className="card overflow-hidden">
            {loading ? (
                    ...
                    <tbody>
                      {data.results.map((product) => (
                        <tr key={product.id}>
                          ...
                          <td>{behaviorLabel(product.inventory_behavior)}</td>
                          {canViewCosts && (
                            <td>
                              {formatDecimalBRL(product.branch_stock?.unit_cost || product.cost || "0")}
                            </td>
                          )}
                          <td>
                            {product.branch_stock?.applicable ? (
                              <>
                                <strong>
                                  {formatQuantity(product.branch_stock.current_quantity || "0")}{" "}
                                  {product.branch_stock.unit?.toUpperCase()}
                                </strong>
                                <span className="block text-[10px] text-muted">
                                  mínimo{" "}
                                  {formatQuantity(product.branch_stock.minimum_quantity || "0")}
                                </span>
                              </>
                            ) : (
                              <span className="text-muted">Não controla</span>
                            )}
                          </td>
                          <td>{formatDecimalBRL(product.sale_price)}</td>
                          <td>
                            {product.archived_at ? (
              ...
              <EmptyState
                ...
                description="Crie produtos para montar o catálogo da empresa."
              />
            )}
          </section>
        </div>
      ...
      <ProductEditorFrame
        open={open}
        page={isDetail}
        title={editing ? `${editing.name} #${editing.id}` : "Novo produto"}
        description={
          editing
            ? `Código ${editing.internal_code} · ${behaviorLabel(editing.inventory_behavior)}`
            : "Cadastre os dados principais do produto."
        }
        actions={
          isDetail ? (
            <div className="flex flex-wrap gap-2">
              ...
              {editing && canDuplicate && (
                <Button
                  type="button"
                  variant="secondary"
                  onClick={() => productActionsRef.current?.openDuplicate()}
                >
                  Duplicar
                </Button>
              )}
              ...
              {editing && canStatus && (
                <Button
                  ...
                  disabled={saving}
                >
                  {editing.status === "active" ? "Inativar" : "Ativar"}
                </Button>
              )}
            </div>
          ) : undefined
        ...
        onClose={() =>
          !saving && (isDetail ? router.push("/produtos") : setOpen(false))
        }
      >
        {isDetail && editing && (
          <>
            ...
            >
              {productTabs.map(([value, label]) => (
                <button
                  ...
                  className={`shrink-0 border-b-2 px-4 py-3 text-xs font-semibold transition ${detailTab === value ? "border-primary text-primary" : "border-transparent text-muted hover:text-fg"}`}
                >
                  {label}
                </button>
              ))}
        ...
        <form id="product-editor-form" onSubmit={submit}>
          <fieldset
            disabled={!!editing && !canChange}
            className="grid gap-5 p-5 disabled:opacity-75 sm:grid-cols-2 lg:grid-cols-3 sm:p-6"
          >
            ...
            {(!isDetail || !editing || detailTab === "data") && (
              <>
                ...
                <Field
                  ...
                  optional={!editing}
                  error={fieldError(fields, "internal_code")}
                >
                  <Input
                    required={!!editing}
                    value={form.internal_code}
                    onChange={(event) =>
                      update("internal_code", event.target.value)
                    }
                    placeholder={
                      editing ? "Código obrigatório" : "Gerado automaticamente"
                    }
                  />
                </Field>
                <Field label="SKU" optional error={fieldError(fields, "sku")}>
                  <Input
                    value={form.sku}
                ...
                <Field label="Categoria" error={fieldError(fields, "category")}>
                  <Select
                    required
                    value={form.category || ""}
                    onChange={(event) =>
                ...
                <Field label="Comportamento de estoque">
                  <Select
                    value={form.inventory_behavior}
                    onChange={(event) =>
                      update(
                        "inventory_behavior",
                        event.target.value as InventoryBehavior,
                      )
                    }
                    disabled={!!editing}
                  >
                    <option value="direct">Controla estoque próprio</option>
                    <option value="none">Não controla estoque</option>
                    <option value="components" disabled={!canCompose}>
                      Baixa componentes
                    </option>
                  </Select>
                  <span className="mt-1 block text-[10px] leading-4 text-slate-400">
                    {editing
                      ...
                      : !canCompose
                        ? "Produtos por componentes exigem permissão para configurar composição."
                        : "Define como o estoque será controlado."}
                  </span>
                </Field>
              </>
            )}
              ...
              <>
                ...
                <Field label="Preço de venda">
                  <MoneyInput
                    ...
                    onValueChange={(next) => {
                      touched.current.sale_price = true;
                      update("sale_price", next);
                    }}
                  />
                  {form.inventory_behavior === "components" && (
                    <span className="mt-1 flex items-center justify-between text-[10px] text-slate-400">
                      ...
                      {canChangePrice && (
                        <button
                          ...
                          onClick={() => {
                            touched.current.sale_price = true;
                            update("sale_price", suggestionSale);
                          }}
                        >
                          Usar sugestão
                        </button>
            ...
            {(!isDetail || !editing || detailTab === "data") && (
              <>
                ...
                <label className="flex items-center gap-3 self-end pb-3 text-xs font-semibold">
                  <input
                    type="checkbox"
                    className="size-4 accent-primary"
                    checked={form.is_sellable}
                    onChange={(event) =>
                      update("is_sellable", event.target.checked)
                    }
                  />
                ...
                <label className="flex items-center gap-3 self-end pb-3 text-xs font-semibold">
                  <input
                    ...
                    onChange={(event) =>
                      update("is_favorite", event.target.checked)
                    }
                  />
                  Produto favorito
                </label>
                ...
                <fieldset className="sm:col-span-2 lg:col-span-3 rounded-lg border border-subtle p-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label className="flex items-center gap-2 text-xs font-semibold">
                      <input
                        type="checkbox"
                        checked={form.participates_in_service_fee}
                        onChange={(event) =>
                          update(
                    ...
                    <label className="flex items-center gap-2 text-xs font-semibold">
                      <input
                        type="checkbox"
                        checked={form.participates_in_commission}
                        onChange={(event) =>
                          update(
                ...
                <div className="sm:col-span-2 lg:col-span-3 rounded-lg border border-slate-200 p-4">
                    ...
                    <div>
                      ...
                      {!canCompose && (
                        <p className="mt-1 text-[10px] text-danger">
                          Sem permissão para configurar composição, esta seção é
                          somente leitura.
                        </p>
                      )}
                    </div>
                  ...
                  <div className="space-y-3">
                    {components.map((component, index) => {
                      ...
                      return (
                        <div
                          key={`${component.component_product}-${index}`}
                          className="grid gap-2 sm:grid-cols-[1fr_10rem_auto]"
                        >
                          <Select
                            required
                            value={component.component_product || ""}
                            onChange={(event) =>
                              updateComponent(
                                index,
                                "component_product",
                          ...
                          <div>
                            {candidate?.unit === "un" ? (
                              <IntegerInput
                                ...
                                onValueChange={(next) =>
                                  updateComponent(index, "quantity", next)
                                }
                                disabled={!canCompose}
                              />
                            ) : (
                            ...
                            <span className="mt-1 block text-[10px] text-slate-400">
                              {displayComponent(component, candidates)}
                            </span>
                          </div>
                          <button
                            type="button"
                  ...
                  <div className="mt-5 border-t border-subtle pt-4">
                    <div className="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <h4 className="text-xs font-bold">
                          Consumo exato de conteúdo
                        </h4>
                        <p className="mt-1 text-[10px] text-muted">
                          Somente insumos fracionáveis, sempre em mL ou g. Nunca
                          em UN fracionária.
                        </p>
                      </div>
                      <Button
                    ...
                    <div className="space-y-3">
                      {fractionComponents.map((component, index) => {
                        const candidate = fractionCandidates.find(
                          (item) => item.id === component.component_product,
                        );
                        const unit =
                          candidate?.fraction_config?.content_unit ||
                          component.content_unit;
                        return (
                          <div
                            key={`fraction-${component.component_product}-${index}`}
                            className="grid gap-2 sm:grid-cols-[1fr_10rem_auto]"
                          >
                            <Select
                              required
                            ...
                            >
                              <option value="">
                                Selecione o insumo fracionável
                              </option>
                              {fractionCandidates
                                .filter(
                                  (item) =>
                                    ...
                                    !components.some(
                                      (normal) =>
                                        normal.component_product === item.id,
                                    ),
                                )
                                .map((item) => (
                                  <option key={item.id} value={item.id}>
                            ...
                            <div>
                              <Input
                                ...
                                min="0.000000001"
                                step="0.000000001"
                                value={component.content_quantity}
                                disabled={!canCompose}
                                onChange={(event) =>
                                  updateFractionComponent(
                                    index,
                            ...
                            <button
                              type="button"
                              className="icon-button"
                              aria-label="Remover consumo exato"
                              disabled={!canCompose}
                              onClick={() =>
                                setFractionComponents((value) =>
                                  value.filter(
          ...
          {!isDetail && (
            <div className="flex justify-end gap-2 border-t border-slate-100 px-5 py-4">
              <Button
                ...
                variant="secondary"
                onClick={() =>
                  isDetail ? router.push("/produtos") : setOpen(false)
                }
                disabled={saving}
              >
              ...
              {(!editing || canChange) && (
                <Button type="submit" loading={saving}>
                  {editing ? "Salvar dados gerais" : "Salvar produto"}
                </Button>
              )}
            </div>
          )}
        </form>
        {editing && currentCompany && currentBranch && (
          <ProductV26Sections
            product={editing}
            ...
            permissions={{
              ...
              viewModifiers: hasPermission(permissions.viewModifiers),
            ...
            onDuplicated={(created) => {
              if (isDetail) router.push(`/produtos/${created.id}`);
              else {
                setOpen(false);
                setSuccess(
                  `Produto duplicado como “${created.name}” (${created.internal_code}). SKU e código de barras foram regenerados com segurança.`,
                );
                void load();
              }
            }}
        ...
        {isDetail && editing && (
          <div className="flex items-center justify-between border-t border-subtle p-4 sm:px-6">
            <Button
              type="button"
              variant="secondary"
      ...
      <ConfirmDialog
        open={!!confirming}
        title={`${confirming?.status === "active" ? "Inativar" : "Ativar"} produto`}
      ...
      <ConfirmDialog
        open={!!archiving}
        title={
          archiving?.archived_at ? "Restaurar produto" : "Arquivar produto"
        }
        message={
          archiving?.archived_at
            ? `Restaurar “${archiving?.name || ""}” para a operação?`
            : `Arquivar “${archiving?.name || ""}”? Vendas, pedidos, movimentações, compras e auditoria serão preservados.`
        }
        confirmLabel={archiving?.archived_at ? "Restaurar" : "Arquivar"}
        danger={!archiving?.archived_at}
        loading={saving}
