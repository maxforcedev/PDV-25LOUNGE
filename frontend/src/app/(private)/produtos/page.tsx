...
import { Heart, Layers3, ListFilter, Pencil, Plus, Power, Search, Trash2, X } from "lucide-react";
import { AdminGuard } from "@/components/admin-guard";
import { PageHeader } from "@/components/page-header";
import { Alert, Button, ConfirmDialog, EmptyState, Field, Input, Modal, Pagination, Select, StatusBadge, TableLoading, Textarea } from "@/components/ui";
import { fieldError } from "@/lib/format";
import { ApiError, http } from "@/lib/http";
import { permissions } from "@/lib/permissions";
import { useAuth } from "@/providers/auth-provider";
import type { Category, InventoryBehavior, Paginated, Product, ProductComponent } from "@/types";

type ProductForm = { company: number; category: number; name: string; description: string; internal_code: string; barcode: string; unit: string; cost?: string; sale_price: string; is_sellable: boolean; is_favorite: boolean; inventory_behavior: InventoryBehavior; image: string };
...
const blank = (company = 0, includeCost = false): ProductForm => ({ company, category: 0, name: "", description: "", internal_code: "", barcode: "", unit: "un", ...(includeCost ? { cost: "0.00" } : {}), sale_price: "0.00", is_sellable: true, is_favorite: false, inventory_behavior: "direct", image: "" });
...
function Products() {
  ...
  function show(product?: Product) {
    if (product ? !canChange : !canAdd) return;
    setEditing(product || null);
    setForm(product ? { company: product.company, category: product.category, name: product.name, description: product.description || "", internal_code: product.internal_code, barcode: product.barcode || "", unit: product.unit, ...(canViewCosts && typeof product.cost === "string" ? { cost: product.cost } : {}), sale_price: product.sale_price, is_sellable: product.is_sellable, is_favorite: product.is_favorite, inventory_behavior: product.inventory_behavior, image: product.image || "" } : blank(currentCompany?.id, canViewCosts));
    setComponents(product?.components || []);
    touched.current = { cost: !!product, sale_price: !!product };
    setFields({}); setError(""); setOpen(true);
  }
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (form.inventory_behavior === "components" && !canCompose && !editing) throw new ApiError("A permissão products.configure_composition é necessária para criar um produto por componentes.", 403);
      if (editing) {
      ...
      setOpen(false);
      setSuccess(editing ? editing.inventory_behavior === "components" && !canCompose ? "Produto atualizado. A composição permaneceu inalterada porque falta products.configure_composition." : "Produto atualizado com sucesso." : "Produto criado com sucesso.");
      await load();
    } catch (caught) {
  ...
  return <>
    <PageHeader title="Produtos" description={`Catálogo, preços e composição de ${currentCompany?.trade_name || "sua empresa"}.`} action={<Button onClick={() => show()} disabled={!canAdd}><Plus className="size-4" />Novo produto</Button>} />
    <div className="space-y-4 p-4 sm:p-6 lg:p-8">
      ...
      <section className="card overflow-hidden">
         {loading ? <TableLoading /> : data?.results.length ? <><div className="table-wrap"><table className="data-table"><thead><tr><th>Produto</th><th>Categoria</th><th>Comportamento</th><th>Preço</th><th>Venda</th><th>Status</th><th className="text-right">Ações</th></tr></thead><tbody>{data.results.map((product) => <tr key={product.id}><td><strong className="flex items-center gap-1.5">{product.is_favorite && <Heart className="size-3.5 fill-primary text-primary" />}{product.name}</strong><span className="text-[11px] text-slate-400">{product.internal_code}</span></td><td>{product.category_name || "-"}</td><td>{behaviorLabel(product.inventory_behavior)}</td><td>R$ {product.sale_price}</td><td>{product.is_sellable ? "Vendável" : "Insumo"}</td><td><StatusBadge active={product.status === "active"} /></td><td><div className="flex justify-end gap-1"><button className="icon-button" disabled={!canChange} onClick={() => show(product)}><Pencil className="size-4" /></button><button className="icon-button" disabled={!canStatus} onClick={() => setConfirming(product)}><Power className="size-4" /></button></div></td></tr>)}</tbody></table></div><Pagination count={data.count} next={data.next} previous={data.previous} onPage={load} /></> : <EmptyState title="Nenhum produto cadastrado" description="Crie produtos para montar o catálogo da empresa." />}
      </section>
    </div>
    <Modal open={open} title={editing ? "Editar produto" : "Novo produto"} description="Valores decimais são enviados sem conversão para preservar a precisão." onClose={() => !saving && setOpen(false)} size="xl">
      <form onSubmit={submit}>
        <div className="grid gap-5 p-5 sm:grid-cols-2 lg:grid-cols-3 sm:p-6">
          ...
          <Field label="Comportamento de estoque"><Select value={form.inventory_behavior} onChange={(event) => update("inventory_behavior", event.target.value as InventoryBehavior)} disabled={!!editing}><option value="direct">Controla estoque próprio</option><option value="none">Não controla estoque</option><option value="components" disabled={!canCompose}>Baixa componentes</option></Select><span className="mt-1 block text-[10px] leading-4 text-slate-400">{editing ? "Definido no cadastro e não pode ser alterado." : !canCompose ? "Produtos por componentes exigem products.configure_composition." : "Define como o estoque será controlado."}</span></Field>
           {canViewCosts && <Field label="Custo"><Input required inputMode="decimal" value={form.cost || ""} onChange={(event) => { touched.current.cost = true; update("cost", event.target.value); }} />{form.inventory_behavior === "components" && <span className="mt-1 flex items-center justify-between text-[10px] text-slate-400">Sugestão local: R$ {suggestionCost}<button type="button" className="font-bold text-primary" onClick={() => { touched.current.cost = true; update("cost", suggestionCost); }}>Usar sugestão</button></span>}{editing?.suggested_cost && <small className="text-[10px] text-slate-400">Sugestão salva pela API: R$ {editing.suggested_cost}</small>}</Field>}
           <Field label="Preço de venda"><Input required inputMode="decimal" value={form.sale_price} onChange={(event) => { touched.current.sale_price = true; update("sale_price", event.target.value); }} />{form.inventory_behavior === "components" && <span className="mt-1 flex items-center justify-between text-[10px] text-slate-400">Sugestão local: R$ {suggestionSale}<button type="button" className="font-bold text-primary" onClick={() => { touched.current.sale_price = true; update("sale_price", suggestionSale); }}>Usar sugestão</button></span>}{editing?.suggested_sale_price && <small className="text-[10px] text-slate-400">Sugestão salva pela API: R$ {editing.suggested_sale_price}</small>}</Field>
          ...
          {form.inventory_behavior === "components" && <div className="sm:col-span-2 lg:col-span-3 rounded-lg border border-slate-200 p-4">
            <div className="mb-3 flex items-center justify-between gap-3"><div><h3 className="flex items-center gap-2 text-xs font-bold"><Layers3 className="size-4 text-primary" />Composição</h3>{!canCompose && <p className="mt-1 text-[10px] text-danger">Sem products.configure_composition, a composição é somente leitura e não será enviada.</p>}</div><Button type="button" variant="secondary" onClick={addComponent} disabled={!canCompose}><Plus className="size-4" />Componente</Button></div>
             <div className="space-y-3">{components.map((component, index) => { const candidate = candidates.find((item) => item.id === component.component_product); return <div key={`${component.component_product}-${index}`} className="grid gap-2 sm:grid-cols-[1fr_10rem_auto]"><Select required value={component.component_product || ""} onChange={(event) => updateComponent(index, "component_product", Number(event.target.value))} disabled={!canCompose}><option value="">Selecione o insumo</option>{candidates.filter((item) => item.id !== editing?.id).map((item) => <option key={item.id} value={item.id}>{item.name} ({item.internal_code})</option>)}</Select><div><Input required type="number" min="0.001" step={candidate?.unit === "un" ? "1" : "0.001"} value={component.quantity} onChange={(event) => updateComponent(index, "quantity", event.target.value)} disabled={!canCompose} /><span className="mt-1 block text-[10px] text-slate-400">{displayComponent(component, candidates)}</span></div><button type="button" className="icon-button" aria-label="Remover componente" disabled={!canCompose} onClick={() => removeComponent(index)}><Trash2 className="size-4" /></button></div>; })}</div>
          </div>}
        </div>
        <div className="flex justify-end gap-2 border-t border-slate-100 px-5 py-4"><Button type="button" variant="secondary" onClick={() => setOpen(false)} disabled={saving}>Cancelar</Button><Button type="submit" loading={saving}>Salvar produto</Button></div>
      </form>
    </Modal>
