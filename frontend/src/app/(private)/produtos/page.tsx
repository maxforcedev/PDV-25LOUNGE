...
function Products() {
  ...
  return <>
    <PageHeader title="Produtos" description={`Catálogo, preços e composição de ${currentCompany?.trade_name || "sua empresa"}.`} action={<div className="flex flex-wrap gap-2"><Link href="/produtos/precos" className="btn btn-secondary"><DollarSign className="size-4" />Preços por filial</Link><Button onClick={() => show()} disabled={!canAdd}><Plus className="size-4" />Novo produto</Button></div>} />
