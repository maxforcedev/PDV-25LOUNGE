...
import { Clock3, CreditCard, Layers, Pencil, Plus, Search, Trash2, Users } from "lucide-react";
    ...
    <>
      ...
      <div className="space-y-4 p-4 sm:p-6 lg:p-8">
        ...
        <section className="flex flex-col gap-3 rounded-lg border border-subtle bg-surface p-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap gap-2">{(["all", "free", "occupied"] as const).map((value) => <Button key={value} variant={filter === value ? "primary" : "secondary"} className="min-h-10" onClick={() => setFilter(value)}>{({ all: "Todas", free: "Livres", occupied: "Ocupadas" })[value]}</Button>)}</div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {visibleTables.map((table) => {
              ...
              return <section key={table.id} className={`card min-h-60 p-5 ${occupied ? "border-primary/40 bg-primary/5" : "border-success/30"}`}>
                ...
                <div className="mt-6 flex items-end justify-between"><div><strong className="text-xl">{occupied ? formatBRL(table.open_commands_total || "0") : formatBRL("0")}</strong></div><span className="inline-flex items-center gap-1 text-xs text-muted"><Users className="size-3" />{table.seats ? `${table.seats} lugares` : "Sem capacidade"}</span></div>
      ...
      <Modal open={!!deleting} title={`Excluir ${deleting?.name || "mesa"}?`} onClose={() => setDeleting(null)}>
        <div className="space-y-4 p-5"><p className="text-sm text-muted">Ela deixará de aparecer nas operações. O histórico de atendimentos e relatórios será preservado.</p><div className="flex justify-end gap-2 border-t border-subtle pt-4"><Button variant="secondary" onClick={() => setDeleting(null)} disabled={saving}>Cancelar</Button><Button loading={saving} onClick={() => deleting && void deleteTable(deleting)}>Excluir mesa</Button></div></div>
      </Modal>
