...
import { ticketModifierLabel } from "@/lib/ticket-presentation";
...
const statusLabels: Record<string, string> = { issued: "Emitido", partially_used: "Parcialmente usado", used: "Usado", cancelled: "Cancelado" };
const methodLabels: Record<string, string> = { scan: "Scanner", manual: "Manual" };
const initialPeriod = (): PeriodValue => businessMonthToDate();
const quantity = (value: unknown) => formatQuantity(String(value || "0"));
const ticketNumber = (value: unknown) => `#${String(value || "").padStart(4, "0")}`;
const text = (value: unknown) => value == null || value === "" ? "-" : String(value);
const optionLabel = (option: Option) => `${option.name}${option.historical ? " (Histórico)" : ""}`;
...
export function TicketsReport() {
  ...
  const [filters, setFilters] = useState<Record<string, string>>({ date_basis: "issued" });
  const [appliedFilters, setAppliedFilters] = useState<Record<string, string>>({ date_basis: "issued" });
  ...
  async function loadOptions(token: number) {
    try {
      ...
      if (context.current === token) setOptions({ products: [], operators: [], devices: [] });
    }
  }
  ...
  const resetForBranch = useEffectEvent(() => {
    ...
    const nextFilters = { date_basis: "issued", ...Object.fromEntries([...current.entries()].filter(([key]) => !["start_datetime", "end_datetime", "branch", "export", "page", "page_size"].includes(key))) };
  ...
  useEffect(() => { resetForBranch(); }, [currentBranch?.id, allowed]);
  ...
  const isRedeemedBasis = (appliedFilters.date_basis || "issued") === "redeemed";
  return <>
    ...
    <div className="space-y-5 p-4 sm:p-6 lg:p-8">
      ...
      <form className="card space-y-4 p-4" onSubmit={(event) => { event.preventDefault(); void load({ ...period }, { ...filters }); }}>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <Field label="Data base"><Select value={filters.date_basis || "issued"} onChange={(event) => setFilters((current) => ({ ...current, date_basis: event.target.value }))}><option value="issued">Emissão</option><option value="redeemed">Retirada</option></Select></Field>
        ...
        <div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={() => { const next = initialPeriod(); const nextFilters = { date_basis: "issued" }; setPeriod(next); setFilters(nextFilters); void load(next, nextFilters); }}>Limpar</Button><Button type="submit">Aplicar</Button></div>
      </form>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Kpi label={isRedeemedBasis ? "Tickets validados" : "Tickets emitidos"} value={summary.tickets_issued} /><Kpi label="Tickets com retirada" value={summary.tickets_validated} /><Kpi label="Parcialmente usados" value={summary.tickets_partially_used} /><Kpi label="Totalmente usados" value={summary.tickets_used} /><Kpi label={isRedeemedBasis ? "Unidades dos tickets" : "Unidades emitidas"} value={summary.issued_quantity} /><Kpi label="Unidades retiradas" value={summary.redeemed_quantity} /><Kpi label="Unidades disponíveis" value={summary.available_quantity} /><Kpi label="Canceladas sem retirada" value={summary.cancelled_quantity} /></div>
      <section className="card overflow-hidden"><div className="card-header"><div><h2 className="text-sm font-bold">{isRedeemedBasis ? "Tickets retirados" : "Tickets emitidos"}</h2><p className="mt-1 text-[11px] text-muted">{data?.count || 0} tickets no recorte aplicado.</p></div><Ticket className="size-5 text-muted" /></div>
