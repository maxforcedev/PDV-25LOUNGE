  ...
  return <>
    ...
    <div className="space-y-5 p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {kind === "purchases" && <><Kpi label="Compras" value={text(summary.count)} /><Kpi label="Itens" value={text(summary.items_count)} /><Kpi label="Recebidas" value={text(summary.received_count)} />{canViewCosts && <Kpi label="Total a pagar" value={money(summary.payable_total)} />}</>}
      ...
      {statusGroups.length > 0 && <section className="card overflow-hidden"><div className="card-header"><div><h2 className="text-sm font-bold">Resumo por status</h2><p className="mt-1 text-[11px] text-muted">Totais do recorte aplicado.</p></div></div><div className="table-wrap"><table className="data-table"><thead><tr><th>Status</th><th>Registros</th>{kind !== "suppliers" && <th>Valor</th>}</tr></thead><tbody>{statusGroups.map((group) => <tr key={String(group.status)}><td><StatusPill kind={kind === "suppliers" ? "suppliers" : kind} status={group.status} /></td><td>{text(group.count)}</td>{kind !== "suppliers" && <td>{money(group.amount ?? group.payable_total)}</td>}</tr>)}</tbody></table></div></section>}
