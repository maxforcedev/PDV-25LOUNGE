...
> **Versão:** 1.3
> **Data:** 2026-08-15  
> **Status:** PRD técnico — MVP revisado após validação de Produtos/Estoque/Caixa, com RBAC por filial, UX compacta, permissões financeiras de estoque, sangria classificada e consumação interna consolidada  
...
A consumação deve possuir fluxo próprio na UI, mas reutilizar o domínio/transações de Sales.
...
- pode ser `R$ 0,00`;
- pode ser menor que a soma dos preços normais e deve permanecer entre `R$ 0,00` e o valor de referência (`subtotal`) no MVP;
- não deve alterar os preços cadastrados dos Products;
- preservar o valor de referência e o valor efetivamente cobrado;
- quando maior que zero, exigir forma(s) de pagamento válida(s);
- quando `charged_amount > 0`, exigir CashSession aberta e Payment(s) válidos; a forma de pagamento determina apenas o impacto no dinheiro físico;
- `charged_amount=0` não gera Payment.

### Estoque

Usar exatamente as regras existentes:

- `direct` → baixa o Product;
- `none` → não baixa estoque;
- `components` → baixa componentes.

Consumação não deve criar uma "venda fake de R$ 0,00" sem identificação. O registro precisa permanecer distinguível por `operation_type='consumption'` para relatórios futuros.
...
As métricas padrão `vendas do dia`, `valor vendido` e `quantidade de vendas` devem considerar `operation_type='sale'` por padrão. Consumação deve permanecer distinguível e não inflar faturamento comercial. Relatórios específicos de consumação ficam para evolução posterior.
...
- [ ] Valor vendido no dia.
- [ ] Quantidade de vendas.
- [ ] Excluir consumações das métricas comerciais padrão de vendas/faturamento.
- [ ] Situação do caixa.
- [ ] Estoques zerados.
...
- [ ] Validar User operacional sem login.
- [ ] Validar que `user_type` não concede permissão.
- [ ] Validar matriz de permissões por módulo/ação.
- [ ] Validar `inventory.view_stock_kpis`.
- [ ] Validar `inventory.view_stock_costs`.
- [ ] Validar que custos/valor de estoque não vazam na API sem permissão.
...
- [ ] Validar `is_favorite`.
- [ ] Validar filtro Todos/Favoritos/Não favoritos em Products.
- [ ] Validar padrão de filtros compactos e limpeza de filtros.
...
- [ ] Validar histórico visual `saldo anterior → saldo final`.
...
- [ ] Validar sangria.
- [ ] Validar sangria com categoria e beneficiário User sem login.
- [ ] Validar que categoria de sangria não altera indevidamente cálculo de saldo.
...
- [ ] Validar consumação gratuita.
- [ ] Validar consumação cobrada.
- [ ] Validar beneficiário User sem login em consumação.
- [ ] Validar consumação `direct`, `none` e `components`.
- [ ] Validar snapshot de `unit_cost` na consumação.
...
A **Sprint 7.1** deve ser executada antes da Sprint 8 porque corrige o fechamento de caixa e incorpora os ajustes de usuários, permissões, filtros, proteção de custos e sangria encontrados após a validação da Sprint 7.
