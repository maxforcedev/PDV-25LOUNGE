...
export default function SettingsPage() {
  ...
  if (!can("platform.settings.manage")) return <ErrorBlock error={new Error("Seu perfil nao possui acesso as politicas globais.")} />;
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) { return <label className="flex min-h-20 items-center gap-3 border border-line bg-white p-4 text-xs font-bold uppercase tracking-wider"><button type="button" role="switch" aria-checked={checked} className={`flex h-6 w-11 items-center border p-0.5 transition ${checked ? "border-ink bg-ink justify-end" : "border-line bg-[#e5e8e3] justify-start"}`} onClick={() => onChange(!checked)}><span className={`flex size-4 items-center justify-center ${checked ? "bg-signal" : "bg-white"}`}>{checked && <Check size={11} />}</span></button>{label}</label>; }
