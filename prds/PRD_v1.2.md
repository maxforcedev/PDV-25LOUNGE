...
> **Versão:** 1.2  
> **Data:** 2026-08-15  
> **Status:** PRD técnico — MVP revisado após validação de Produtos/Estoque, com RBAC por filial, UX de catálogo/estoque e regras de Caixa/PDV consolidadas  
...
O MVP deve cobrir o núcleo operacional necessário para que uma empresa consiga configurar sua estrutura básica, criar perfis de acesso e permissões com aplicação por filial, organizar categorias na ordem operacional desejada, cadastrar produtos simples, não estocáveis ou compostos, destacar produtos favoritos para o PDV, controlar estoque por filial com estoque mínimo e visão financeira aproximada a custo atual, configurar formas de pagamento, abrir caixa, realizar uma venda, registrar pagamento e troco quando aplicável, aplicar desconto simples autorizado, baixar o estoque correto conforme o comportamento do produto, consultar ou cancelar a venda, movimentar o caixa, fechar o caixa comparando saldo esperado e valor informado e acompanhar indicadores básicos no dashboard.
...
Exemplo: ao filtrar `Category = Cervejas`, os widgets passam a mostrar somente zerados, abaixo do mínimo e valor em estoque da categoria Cervejas.
...
O filtro por Category deve permitir responder, por exemplo, quanto existe em quantidade e valor no grupo `Cervejas`.
...
- backend valida e recalcula `total = subtotal - discount`;
...
**Não implementar:** percentual, cupons, promoções ou motor de regras.
...
- [ ] AC-40 — Visualizar dashboard operacional final.
...
| PDV | 8–9 | venda, PaymentMethod configurável, desconto simples, troco, snapshots e baixa direta/por composição |
...
**Objetivo:** estruturar domínio de venda, PaymentMethod, snapshots, desconto simples e PDV.

- [ ] Criar app `apps/sales`.
- [ ] Criar `Sale`.
- [ ] Criar `SaleItem`.
- [ ] Adicionar snapshots `product_name`, `internal_code`, `unit`, `unit_price`, `subtotal`.
- [ ] Criar `PaymentMethod` por Company.
- [ ] Criar `Payment` referenciando `PaymentMethod`.
- [ ] Adicionar `received_amount` e `change_amount` para dinheiro.
- [ ] Criar migrations.
...
- [ ] Preservar preço histórico e snapshot do item.
- [ ] Implementar desconto monetário simples.
- [ ] Criar permissão específica para aplicar desconto.
- [ ] Criar service de cálculo.
- [ ] Criar tela de PDV.
- [ ] Exibir Branch atual claramente no PDV.
- [ ] Priorizar favoritos.
- [ ] Respeitar ordem configurada das Categories.
- [ ] Buscar somente produtos ativos/vendáveis por nome/código/barcode.
- [ ] Adicionar/remover item no frontend.
- [ ] Alterar quantidade.
- [ ] Exibir subtotal provisório na UI.
- [ ] Permitir desconto em R$ somente quando autorizado.
- [ ] Listar no PDV somente PaymentMethods ativos.
- [ ] Selecionar forma de pagamento.
- [ ] Para dinheiro, permitir informar valor recebido e visualizar troco.
- [ ] Não tratar valor, desconto, comportamento de estoque, composição, troco ou PaymentMethod enviado pela UI como fonte definitiva.
- [ ] Proteger configuração e venda pelas permissões da Branch atual.

**Entrega:** PDV pronto para finalização segura com favoritos, categorias ordenadas, desconto simples, snapshots e troco.
...
- [ ] Validar usuário/permissão efetiva da Branch.
- [ ] Validar Company/Branch.
- [ ] Validar CashSession aberta.
- [ ] Carregar produtos no backend.
- [ ] Validar Product ativo e `is_sellable`.
...
- [ ] Validar desconto sem permissão rejeitado.
- [ ] Validar troco não altera Sale.total nem Payment.amount.
- [ ] Integrar vendas em dinheiro ao saldo esperado da CashSession usando `Payment.amount`.
- [ ] Garantir que `received_amount` não seja somado ao saldo esperado além do `Payment.amount`.
...
- [ ] Exibir snapshots históricos da SaleItem, sem depender do nome/código/unidade atuais do Product.
...
- [ ] Garantir coerência entre alertas do dashboard e regras de situação usadas na tela de Stock.
...
- [ ] Validar mesmo User com perfil diferente em duas Branches da mesma Company.
- [ ] Validar troca de Branch atual sem vazamento de permissões.
- [ ] Validar Category obrigatória.
- [ ] Validar drag-and-drop e persistência da ordem de Categories.
- [ ] Validar `product_count` na listagem de Categories.
- [ ] Validar seção Produtos relacionados no detalhe/edição de Category.
- [ ] Validar Product sem internal_code recebendo código gerado.
- [ ] Validar unicidade concorrente do código.
- [ ] Validar `is_favorite`.
- [ ] Validar unidade por select.
- [ ] Validar produto `direct`.
- [ ] Validar produto `none`.
- [ ] Validar insumo `is_sellable=False`.
- [ ] Validar produto `components` e composição.
- [ ] Validar sugestões de custo e preço do combo aplicadas aos campos existentes quando ainda não preenchidos.
- [ ] Validar que `cost` e `sale_price` editados manualmente não são sobrescritos ao alterar composição.
- [ ] Validar quantidade de componente exibida com unidade, por exemplo `5 UN` em vez de `5.000`.
- [ ] Validar bloqueio de composição recursiva.
- [ ] Validar Entrada/Saída/Ajuste.
- [ ] Validar movimentação manual com motivo vazio e confirmar trilha de auditoria completa.
- [ ] Validar formatação `20.000 -> 20`.
- [ ] Validar estoque mínimo por Branch.
- [ ] Validar bloqueio de estoque negativo.
- [ ] Validar filtros de Stock por Branch/Category/nome/status/situação/comportamento.
- [ ] Validar widget de zerados.
- [ ] Validar widget de abaixo do mínimo.
- [ ] Validar widget de valor em estoque.
- [ ] Validar que widgets mudam junto com filtros.
- [ ] Filtrar Category `Cervejas` e validar quantidade/valor somente do grupo.
- [ ] Validar custo unitário e custo total na listagem.
...
- [ ] Validar sangria.
- [ ] Validar saldo esperado antes de vendas.
- [ ] Validar fechamento com esperado/informado/diferença.
- [ ] Validar venda com produto `direct`.
- [ ] Validar venda com produto `none`.
- [ ] Validar venda com produto `components`.
- [ ] Validar favoritos e ordem das categorias no PDV.
- [ ] Validar desconto monetário com permissão.
- [ ] Validar rejeição de desconto sem permissão.
- [ ] Validar pagamento com método ativo.
- [ ] Validar rejeição de método inativo.
- [ ] Validar valor recebido e troco em dinheiro.
- [ ] Validar que troco não infla Payment.amount nem caixa esperado.
- [ ] Validar snapshots de SaleItem.
- [ ] Alterar nome/código/unidade/preço de Product e confirmar histórico intacto.
...
- [ ] Validar fechamento após vendas em dinheiro.
- [ ] Validar dashboard.
...
10. fechamento de caixa auditável;
11. frontend rápido e claro;
12. arquitetura capaz de evoluir sem reescrever o núcleo.
...
**Empresa → Filial/Perfil → Usuário → Categoria ordenada → Produto/Composição → Estoque/Mínimo/Valor → Formas de Pagamento → Caixa → Venda/Desconto → Pagamento/Troco → Baixa Direta/Componentes → Consulta/Cancelamento Histórico → Fechamento Esperado x Informado → Dashboard.**

A **Sprint 6.1** deve ser executada antes da Sprint 7 porque consolida correções descobertas durante o teste real de Produtos e Estoque. Ela não apaga o histórico das Sprints 4.1, 5 e 6.
...
- Validações manuais realizadas após as Sprints 4.1, 5 e 6, incorporadas como requisitos da revisão 1.2.
