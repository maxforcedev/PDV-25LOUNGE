...
## Adendo pós-Sprint 12.4 — Decisões finais pré-deploy

> **Regra de preservação histórica:** este adendo foi criado para registrar decisões posteriores à Sprint 12.4 **sem alterar, desmarcar, reescrever ou invalidar nenhum `[X]`, `[!]`, critério de aceite ou texto histórico já executado**. Quando houver conflito de terminologia, UX ou fórmula entre uma regra antiga e este adendo, **este adendo é a regra mais recente para a implementação futura**, enquanto o conteúdo anterior permanece como histórico do que foi definido/executado naquele momento.
>
> **Limite de escopo:** estas decisões ainda pertencem ao fechamento funcional pré-deploy. Não iniciar VPS, Swarm, Traefik, Cloudflare, HTTPS, registry, secrets, runtime de produção ou deploy antes da validação desta rodada e de autorização expressa do responsável pelo produto.
>
> **Central de Ajuda:** a experiência atual de `/ajuda` é considerada aprovada. Preservar estrutura, conteúdo, visual e comportamento, salvo correção objetiva de regressão.

### A. Landing Page — foco comercial e conversão

A estrutura/template visual atual da Landing Page deve ser preservada como base, porém a comunicação deve deixar de parecer genérica, descritiva ou produzida por template/IA. A Landing deve responder de forma direta:

- **qual problema real o CORE PDV resolve;**
- **por que o estabelecimento perde controle/dinheiro sem uma operação integrada;**
- **por que vale a pena comprar/assinar o sistema;**
- **qual ganho operacional e gerencial o proprietário obtém ao centralizar vendas, caixa, estoque, equipe, consumação, comissão e resultado.**

Direção obrigatória da copy:

- priorizar dor → consequência → solução → benefício → chamada para ação;
- vender resultado e controle, não apenas enumerar funcionalidades;
- usar as funcionalidades como **prova da solução**, e não como texto principal da proposta de valor;
- destacar dores concretas do público-alvo, como divergência de caixa, estoque sem rastreabilidade, comissão difícil de conferir, consumação sem controle, desconto sem auditoria e dificuldade para entender quanto realmente sobrou da operação;
- deixar claro que o CORE PDV conecta operação e gestão em uma única fonte de verdade;
- criar CTA comercial evidente e repetido nos pontos corretos da página;
- evitar frases vagas como “gestão inteligente”, “solução completa”, “revolucione seu negócio” ou equivalentes sem benefício concreto;
- evitar aparência textual de template genérico/IA;
- manter pt-BR natural, comercial e objetivo.

É proibido inventar para aumentar conversão:

- clientes inexistentes;
- depoimentos falsos;
- números de faturamento/estabelecimentos sem fonte real;
- integrações ainda não existentes;
- planos/preços ainda não definidos;
- selos, parceiros ou homologações inexistentes;
- urgência/escassez artificial.

### B. Regra global dos atalhos de período

Os atalhos rápidos de período passam a ter comportamento **imediato em todo o sistema**.
...
- `Últimos 15 dias`;
- `Últimos 30 dias`;
- `Sessão atual`, quando aplicável;

devem:

1. preencher o intervalo correspondente;
2. atualizar `start_datetime`/`end_datetime`;
3. aplicar o filtro imediatamente;
4. atualizar a URL/query params quando a tela utilizar estado navegável;
5. disparar a nova consulta ao backend sem exigir um segundo clique em `Aplicar`.

Regra mandatória:

> **Clicou em um atalho de período pronto → aplicou.**

O botão `Aplicar` permanece para:

- período `Personalizado`;
- edição manual de data/hora;
- demais filtros avançados que dependam de confirmação conjunta.

Não pode existir o fluxo:

```text
Hoje
→ usuário precisa clicar em Aplicar
```

O comportamento deve ser consistente no Dashboard, Relatórios, Vendas, Consumação, Caixa, Auditoria, Estoque/Movimentações e demais telas que utilizem atalhos equivalentes.

### C. Semântica financeira final — nomenclatura simplificada

A V1 passa a utilizar uma nomenclatura gerencial simplificada para evitar a coexistência confusa de `Total recebido em vendas`, `Total recebido operacional`, `Receita operacional recebida` e rótulos equivalentes na interface principal.

#### C.1 Componentes

Para vendas comerciais válidas do recorte:

```text
Faturamento de vendas
= receita dos produtos após promoções, desconto por item e desconto da conta
```

Para consumações válidas:

```text
Consumação cobrada
= valor efetivamente cobrado nas consumações
```

A nova regra principal é:
...
A taxa de serviço **não** entra no `Faturamento efetivo`.
...
#### C.2 Regra de nomenclatura na UI

Na experiência gerencial principal, utilizar **um único rótulo final `Total recebido`**.

Deixar de usar como KPIs finais concorrentes:
...
quando estiverem representando apenas etapas intermediárias da mesma reconciliação.

Esses componentes podem continuar existindo tecnicamente no backend ou em demonstrativos detalhados quando necessários para auditoria/reconciliação, mas a UI deve apresentar a decomposição de forma clara e terminar em um único `Total recebido`.

Exemplo:

```text
Faturamento de vendas        R$ 801,01
Consumação cobrada           R$  50,00
---------------------------------------
Faturamento efetivo          R$ 851,01

Taxa de serviço              R$  73,30
---------------------------------------
Total recebido               R$ 924,31
```

#### C.3 Reconciliação com pagamentos

Para o mesmo conjunto de operações válidas e o mesmo recorte:
...
- reversões;
- filtros;
- escopo Company/Branch;
- arredondamento Decimal centralizado.

Troco não aumenta `Total recebido`.

Comissão não reduz `Faturamento efetivo` nem `Total recebido`; comissão continua sendo custo operacional usado no Resultado estimado.

Consumação gratuita (`charged_amount = 0`) não aumenta `Faturamento efetivo` nem `Total recebido`, mas seu custo histórico continua podendo afetar o Resultado estimado.

#### C.4 Ticket médio

O `Ticket médio` padrão continua sendo comercial:

```text
Ticket médio
= Faturamento de vendas / quantidade de vendas comerciais válidas
```

Consumação não entra silenciosamente no denominador nem transforma o indicador em “ticket operacional”. Caso no futuro exista outro indicador que inclua consumação, ele deve possuir nome próprio.

### D. Caixa e Resultado — leitura gerencial final

No Dashboard e nos relatórios, substituir o rótulo gerencial `Recebimento operacional` pelo novo `Total recebido`.
...
A margem estimada deve usar:

```text
Margem estimada (%)
= Resultado estimado / Total recebido × 100
```

quando `Total recebido > 0`.

Regras:

- exibir `Total recebido`, não `Recebimento operacional`, no card/resumo;
- `Resultado estimado` continua sendo gerencial e não uma DRE contábil oficial;
- sangria continua não sendo despesa automaticamente;
- custos de consumação válidos devem continuar considerados no Resultado, inclusive quando a consumação tiver sido gratuita;
- o drill-down `Ver resultado` deve mostrar claramente a ponte `Faturamento efetivo → Taxa → Total recebido → Custos/Despesas → Resultado`;
- nenhuma tela pode aplicar fórmula própria diferente para o mesmo recorte.

### E. Dashboard — composição final

O Dashboard deve ser revisado novamente. Nenhum `[X]` histórico anterior é alterado, porém esta composição passa a ser a direção mais recente da tela.

#### E.1 Formas de pagamento

`Formas de pagamento` deve utilizar **gráfico de donut/pizza**, com preferência visual por donut.

Exibir, quando houver espaço e permissão:

- nome da forma;
- valor recebido;
- percentual sobre o `Total recebido`;
- total geral;
- tooltip;
- drill-down para Recebimentos filtrado pela forma selecionada.

O gráfico deve reconciliar com o `Total recebido` do mesmo período/escopo.

#### E.2 Últimas vendas

`Últimas vendas` deve:

- ser o **último bloco do Dashboard**;
- ordenar da operação mais recente para a mais antiga;
- apresentar **10 operações por página**;
- possuir paginação funcional;
- preservar filtros globais aplicáveis;
- permitir abrir o detalhe da operação;
- possuir ação para acessar a listagem/relatório completo.

Não carregar uma lista indefinida de operações na página inicial.

#### E.3 Faturamento por atendente

A antiga leitura de `Faturamento por atendente` deve assumir formato de **ranking gerencial**, e não gráfico genérico simples.

Exemplo de informações:

```text
1º  João      R$ 4.820,00   47 vendas   Ticket R$ 102,55
2º  Maria     R$ 3.940,00   41 vendas   Ticket R$  96,10
3º  Lucas     R$ 2.780,00   29 vendas   Ticket R$  95,86
```

Quando autorizado, também pode exibir comissão do atendente.

Regras:

- ranking baseado em `seller_user`;
- não atribuir comissão ao `created_by` apenas por ter processado a venda;
- permitir drill-down no atendente;
- usar hierarquia visual de colocação, valor e desempenho;
- pode utilizar barra horizontal de apoio, mas a leitura principal deve ser de ranking.

#### E.4 Mapa de calor

O `Mapa de calor` continua obrigatório, porém sua posição passa a ser fixa:

> **Mapa de calor = penúltimo bloco do Dashboard.**

Ele deve aparecer **imediatamente acima de `Últimas vendas`**.

Nenhum outro bloco deve ser inserido entre:
...
Todos os demais blocos devem ficar acima do Mapa de calor. A ordem intermediária pode ser ajustada por responsividade, mas a hierarquia recomendada é:

```text
Filtros globais
KPIs principais reconciliados
Caixa e Resultado
Produtos mais vendidos + Formas de pagamento
Ranking de atendentes
Comparativo atual × anterior
Estoque / alertas operacionais
Mapa de calor
Últimas vendas
```

Regras já existentes continuam válidas:

- não utilizar gráfico de linha/área como visualização principal;
- manter drill-down útil;
- respeitar permissões;
- usar design system;
- suportar light/dark;
- preservar filtros no drill-down.

### F. Revisão de corretude antes do deploy

Como foram identificadas novas dúvidas após a validação anterior, a implementação atual do Dashboard e dos agregados financeiros deve ser **rechecada no código e em runtime**, sem assumir que um `[X]` histórico prova a corretude do estado atual.
...
- Resultado estimado;
- Margem estimada;
- filtros de Branch/Category/período;
- atalhos imediatos de período;
- cancelamentos/estornos/reversões;
- navegação e drill-down.

Para o mesmo escopo, Dashboard, Relatórios e exportações não podem retornar totais incompatíveis.

---

## Sprint 12.5 — Ajustes Finais Pré-Deploy: Landing, Dashboard e Semântica Financeira

**Objetivo:** implementar exclusivamente as decisões do adendo pós-Sprint 12.4, preservando integralmente todas as sprints históricas e fechando a última rodada funcional antes da autorização da Sprint 13.

### Regra mandatória

> **Não editar checkboxes, textos de entrega ou histórico das Sprints 12.3 e 12.4. Todos os novos trabalhos desta rodada ficam nesta Sprint 12.5.**

> **Não executar deploy nem antecipar tarefas exclusivas da Sprint 13.**

### 1. Landing Page

- [X] Preservar o template/layout visual atual como base.
- [ ] Reescrever hero, subtítulos, blocos de valor e CTAs com foco real em conversão.
- [X] Explicar claramente por que um bar/restaurante/lounge deve comprar/assinar o CORE PDV.
- [X] Organizar a narrativa em dor → consequência → solução → benefício → CTA.
- [X] Usar funcionalidades como prova do benefício, não como lista genérica.
- [X] Remover frases vagas/genéricas com aparência de template/IA.
- [X] Não inventar clientes, números, depoimentos, preços, integrações ou homologações.
- [X] Preservar `/ajuda` sem redesign ou reescrita desnecessária.

### 2. Atalhos globais de período

- [X] Fazer `Hoje`, `Ontem`, `Últimos 7 dias`, `Últimos 15 dias`, `Últimos 30 dias` e `Sessão atual` aplicarem imediatamente ao clique.
- [X] Atualizar datas/horas, query params e consulta ao backend no mesmo clique.
- [X] Remover a necessidade de clicar em `Aplicar` depois de um atalho pronto.
- [X] Manter `Aplicar` para `Personalizado`, edição manual de data/hora e filtros avançados.
- [X] Aplicar o mesmo comportamento em todas as telas que possuem atalhos de período.
- [X] Garantir que KPIs, gráficos, tabelas e exportações utilizem o período efetivamente aplicado.

### 3. Semântica financeira consolidada

- [X] Implementar `Faturamento efetivo = Faturamento de vendas + Consumação cobrada`.
- [X] Garantir que taxa de serviço permaneça fora do Faturamento efetivo.
- [X] Implementar `Total recebido = Faturamento efetivo + Taxa de serviço`.
- [X] Fazer `SUM(Payment.amount)` reconciliar com `Total recebido` no mesmo recorte, respeitando reversões.
- [X] Unificar a nomenclatura principal da UI em `Total recebido`.
- [X] Remover da leitura gerencial principal a concorrência entre `Total recebido em vendas`, `Total recebido operacional`, `Recebimento operacional` e `Receita operacional recebida`.
- [X] Preservar decomposição técnica/detalhada quando necessária para auditoria e relatórios.
- [X] Manter comissão como custo posterior, sem reduzir faturamento/recebimento.
- [X] Manter Ticket médio padrão restrito às vendas comerciais válidas.

### 4. Caixa e Resultado

- [X] Trocar o rótulo `Recebimento operacional` por `Total recebido` no Dashboard/Resultado quando representar a mesma grandeza.
- [X] Exibir a ponte `Faturamento de vendas + Consumação cobrada = Faturamento efetivo`.
- [X] Exibir `Faturamento efetivo + Taxa de serviço = Total recebido`.
- [X] Calcular Resultado estimado a partir do Total recebido menos CMV, custo de consumação, comissão, despesas que afetam resultado e custo fixo rateado.
- [X] Calcular Margem estimada sobre `Total recebido`.
- [X] Confirmar que consumação gratuita não gera receita, mas seu custo continua afetando Resultado.
- [X] Reconciliar o card/resumo com `/relatorios/resultado`.

### 5. Dashboard — visual e comportamento

- [X] Alterar `Formas de pagamento` para gráfico donut/pizza, preferencialmente donut.
- [X] Fazer o total do donut reconciliar com `Total recebido`.
- [X] Transformar `Faturamento por atendente` em ranking por `seller_user`.
- [X] Exibir no ranking valor, quantidade de vendas e Ticket médio; comissão somente quando autorizada.
- [X] Tornar cada atendente navegável para seu drill-down.
- [X] Fazer `Últimas vendas` usar 10 itens por página com paginação.
- [X] Manter `Últimas vendas` como último bloco do Dashboard.
- [X] Mover `Mapa de calor` para penúltimo bloco.
- [X] Garantir que nenhum bloco fique entre `Mapa de calor` e `Últimas vendas`.
- [X] Manter todos os demais blocos acima do Mapa de calor.
- [X] Preservar a regra de não usar linha/área como visualização principal.
- [ ] Preservar responsividade, dark mode, drill-down e permissões.

### 6. Revisão final de dados do Dashboard

- [X] Auditar novamente os endpoints/agregadores usados pelo Dashboard.
- [X] Conferir os mesmos filtros contra os relatórios dedicados.
- [X] Conferir Faturamento de vendas, Consumação cobrada, Faturamento efetivo, Taxa e Total recebido.
- [X] Conferir Payments por forma e percentual do donut.
- [X] Conferir Ticket médio e quantidade de vendas comerciais.
- [X] Conferir ranking de atendentes por `seller_user`.
- [X] Conferir Resultado e Margem.
- [X] Conferir cancelamentos/estornos/reversões.
- [X] Corrigir qualquer divergência encontrada sem alterar o histórico das sprints anteriores.

### 7. Verificações permitidas

- [X] Executar `python manage.py check`.
- [X] Verificar migrations sem reescrever migrations históricas já aplicadas.
- [X] Executar build/check do frontend quando o ambiente permitir.
- [X] Validar healthcheck quando infraestrutura local necessária estiver disponível.
- [X] Fazer validação manual dos fluxos alterados.
- [X] Não criar/reintroduzir testes automatizados para cumprir esta sprint.

### Gate de saída da Sprint 12.5

A Sprint 12.5 somente pode ser considerada pronta para validação final quando:

- [X] Landing comunica claramente motivo para comprar/assinar, sem claims falsos.
- [X] `/ajuda` permanece preservada.
- [X] Atalhos rápidos de período aplicam imediatamente em todo o sistema.
- [X] Existe uma única leitura gerencial final de `Total recebido`.
- [X] `Faturamento efetivo` inclui vendas + consumação cobrada e exclui taxa.
- [X] `Total recebido` inclui Faturamento efetivo + taxa de serviço.
- [X] Resultado e Margem usam a nova semântica de forma reconciliável.
- [X] Formas de pagamento usam donut/pizza e reconciliam com Total recebido.
- [X] Faturamento por atendente é ranking.
- [X] Últimas vendas possuem paginação de 10 itens.
- [X] Mapa de calor é o penúltimo bloco e Últimas vendas é o último.
- [X] Dashboard e relatórios retornam números compatíveis para o mesmo recorte.
- [X] Nenhuma regressão crítica conhecida permanece.
- [ ] O responsável pelo produto executa a validação manual e autoriza, ou não, o início da Sprint 13.

**Entrega:** refinamento funcional final pré-deploy concluído, sem alterar o histórico das sprints já executadas e sem executar produção automaticamente.
